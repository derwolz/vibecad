...
"""Contract tests for SteveCAD's mutually exclusive MCP control mode."""
...
import SteveCADMCP as mcp
from SteveCADMCPToolNames import (
    MCPToolNameError,
    mcp_tool_wire_name,
...
class _DeterministicController(mcp.SteveCADControlModeController):
    def __init__(self) -> None:
        super().__init__()
...
def test_mcp_wire_names_are_concise_pascal_case() -> None:
    ...
    assert mcp_tool_wire_name("stevecad.manage_document") == "ManageDocument"
...
def test_mcp_ipc_address_rejects_live_owner_and_removes_stale_socket(
    ...
    with pytest.raises(RuntimeError, match="another SteveCAD instance"):
...
def test_application_shutdown_waits_for_mcp_lifecycle_threads() -> None:
    ...
    controller = mcp.SteveCADControlModeController()
...
def test_ipc_collision_does_not_disable_persisted_mcp_preference() -> None:
    ...
    controller._handle_server_event(
        3,
        {
            ...
            "error": "another SteveCAD instance owns the local broker",
        },
    )
...
def test_host_surface_is_exact_internal_schema_plus_controller_tools() -> None:
    ...
    modules = {
        ...
        "SteveCADCore": SimpleNamespace(get_service=lambda: service),
        "SteveCADModelingSurface": SimpleNamespace(
...
def test_host_runner_forwards_the_exact_frozen_provider_surface() -> None:
    ...
    with mock.patch.dict(sys.modules, {"SteveCADSession": session_module}):
...
def test_read_operation_uses_existing_runner_without_live_surface_refresh() -> None:
    ...
    with mock.patch.dict(sys.modules, {"SteveCADProvider": provider}):
        result = session.call_tool(
...
def test_connection_configuration_is_a_clean_stdio_launch_specification() -> None:
    ...
    assert configuration["args"][-1].endswith("SteveCADMCPStdio.py")
