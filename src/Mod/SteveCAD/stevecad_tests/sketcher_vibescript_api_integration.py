...
def _exercise_lifecycle(root: Path) -> None:
    ...
    assert profile.SteveCADVibeScriptRevision == accepted["accepted_revision"]
    ...
    assert profile.SteveCADVibeScriptRevision == updated["accepted_revision"]
...
def main() -> int:
    ...
    root = Path(tempfile.mkdtemp(prefix="stevecad-sketcher-production-"))
    try:
