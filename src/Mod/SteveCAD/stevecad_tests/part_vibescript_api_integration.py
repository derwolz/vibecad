...
from SteveCADSession import (  # noqa: E402
...
def _exercise_every_export(api) -> dict[str, dict]:
    ...
    reference_root = Path(tempfile.mkdtemp(prefix="stevecad-part-reference-api-"))
    try:
...
def _exercise_isolated_lifecycle(root: Path, pack) -> dict:
    ...
    for object_name in identities.values():
        ...
        assert output.SteveCADDerivedState == "accepted"
    ...
    for object_name in identities.values():
        ...
        assert obj.SteveCADVibeScriptRevision == accepted["accepted_revision"]
    ...
    live_revision_before_build = str(
        document.getObject(identities["Housing"]).SteveCADVibeScriptRevision
    )
    ...
    assert (
        str(document.getObject(identities["Housing"]).SteveCADVibeScriptRevision)
    ...
    for object_name in identities.values():
        output = document.getObject(object_name)
        assert output.SteveCADDerivedState == "stale"
        assert "NativeSeed.Shape" in output.SteveCADStaleReason
    ...
    assert housing_object.SteveCADVibeScriptRevision == accepted["accepted_revision"]
    ...
    for type_id, name, _should_be_stale in engineering_consumer_specs:
        ...
        consumer.SteveCADTestSource = housing_object
    ...
    for consumer, (_type_id, name, should_be_stale) in zip(
        ...
        assert consumer.SteveCADTestSource is housing_object
        assert name in touched_names
        if should_be_stale:
            assert name in stale_names
            assert consumer.SteveCADDerivedState == "stale"
            assert "regenerate this derived result" in consumer.SteveCADStaleReason
        else:
    ...
    for _type_id, name, should_be_stale in engineering_consumer_specs:
        ...
        assert consumer.SteveCADTestSource is reopened_housing
        if should_be_stale:
            assert consumer.SteveCADDerivedState == "stale"
    ...
    for _type_id, name, should_be_stale in engineering_consumer_specs:
        consumer = reopened.getObject(name)
        assert consumer.SteveCADTestSource is reopened.getObject(remaining_identities["Housing"])
        if should_be_stale:
    ...
    from SteveCADVibeScriptDomains import get_domain_adapter
...
def main() -> int:
    ...
    root = Path(tempfile.mkdtemp(prefix="stevecad-part-production-"))
    try:
