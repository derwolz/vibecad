...
import SteveCADAgentCli as cli
import SteveCADAgentControl as control
...
def _development_attestations(tmp_path, monkeypatch):
    ...
    source_root = repo / "src" / "Mod" / "SteveCAD"
    runtime_root = (
        repo
        / "package"
        / "rattler-build"
    ...
    installed_root = runtime_root / "Mod" / "SteveCAD"
    executable = runtime_root / "bin" / "SteveCAD.exe"
    ...
    attestation_root = repo / ".stevecad-dev" / "attestations"
    executable.parent.mkdir(parents=True, exist_ok=True)
    installed_root.mkdir(parents=True, exist_ok=True)
    attestation_root.mkdir(parents=True, exist_ok=True)
    executable.write_bytes(b"exact-stevecad-executable")
    ...
    module_specs = (
        ...
        (
            ...
            source_root / "SteveCADAgentControl.py",
            installed_root / "SteveCADAgentControl.py",
        ),
        (
            "SteveCADAgentCli.py",
            source_root / "SteveCADAgentCli.py",
            installed_root / "SteveCADAgentCli.py",
        ),
        (
            "SteveCADGui.py",
            source_root / "SteveCADGui.py",
            installed_root / "SteveCADGui.py",
        ),
        (
            "Invoke-SteveCAD-VisibleTour.ps1",
            repo / "Invoke-SteveCAD-VisibleTour.ps1",
            None,
        ),
        ("Launch-SteveCAD-Dev.ps1", repo / "Launch-SteveCAD-Dev.ps1", None),
    )
    modules = []
    runtime_paths = {}
...
def test_development_runtime_identity_binds_actual_files(tmp_path, monkeypatch) -> None:
    ...
    assert identity["schema"] == "stevecad.dev-runtime-identity.v1"
...
def test_development_runtime_identity_rejects_other_process_qwindows(
    ...
    with pytest.raises(RuntimeError, match="current SteveCAD process"):
        control.development_runtime_identity()
...
def test_development_runtime_identity_rejects_modified_installed_module(
    ...
    attested = _development_attestations(tmp_path, monkeypatch)
    (attested.installed_root / "SteveCADAgentControl.py").write_text(
...
def test_development_runtime_identity_rejects_stale_install_after_source_change(
    ...
    attested = _development_attestations(tmp_path, monkeypatch)
    (attested.source_root / "SteveCADGui.py").write_text(
...
def test_endpoint_and_status_publish_matching_process_runtime_identity(
    ...
    monkeypatch.setenv(
        ...
        str(attested.repo / ".stevecad-dev" / "agent"),
    )
...
def test_status_advertises_additive_operation_tracking_contract(
    ...
    assert status["operation_tracking"] == {
        "schema": "stevecad.dev-operation-tracking.v1",
...
def test_development_token_acl_is_current_user_only_and_exact_path(
    ...
    repo = (tmp_path / "checkout").resolve()
    home = (repo / ".stevecad-dev" / "agent").resolve()
    secured = []
...
def test_only_fail_closed_server_rejects_non_loopback_clients() -> None:
    strict_server = SimpleNamespace(stevecad_fail_closed=True)
    legacy_server = SimpleNamespace(stevecad_fail_closed=False)
...
def test_status_reports_grok_without_secrets(tmp_path, monkeypatch) -> None:
    monkeypatch.setenv(control.AGENT_HOME_ENV, str(tmp_path / "agent-home"))
    import SteveCADGrokAuth as grok
...
def test_ui_ribbon_reports_live_semantic_screen_geometry(monkeypatch) -> None:
    ...
    class Tabs:
        ...
        def tabData(self, index: int):  # noqa: N802 - Qt API spelling
            return ("PartDesignWorkbench", "SteveCADAeroWorkbench")[index]
        ...
        def objectName(self) -> str:  # noqa: N802
            return "SteveCADRibbonTabs"

    tabs = Tabs()
    window = SimpleNamespace(
        findChild=lambda _kind, name: tabs if name == "SteveCADRibbonTabs" else None,
        windowTitle=lambda: "SteveCAD DEV CONTROLLED",
        winId=lambda: 4242,
    )
...
def test_screenshot_captures_the_visible_stevecad_window(tmp_path, monkeypatch) -> None:
    target = tmp_path / "visible-stevecad.png"
    ...
    class Pixmap:
        ...
        def save(self, path: str, image_format: str) -> bool:
            ...
            Path(path).write_bytes(b"fake-visible-stevecad-png")
            return True

    window = SimpleNamespace(
        grab=lambda: Pixmap(),
        windowTitle=lambda: "SteveCAD DEV test",
        winId=lambda: 12345,
    )
    monkeypatch.setattr(
        control,
    ...
    assert payload["capture"]["window_title"] == "SteveCAD DEV test"
...
def test_preferences_open_named_page(monkeypatch) -> None:
    ...
    payload = control.dispatch("preferences")
    assert payload == {"ok": True, "opened": "SteveCAD"}
    assert calls == [("SteveCAD", "SteveCAD")]
...
def test_cli_parses_freecadcmd_argv_and_prefers_http(monkeypatch) -> None:
    ...
    args = cli.build_parser().parse_args(
        cli._argv_for_parser(
            [
                "FreeCADCmd.exe",
                "C:\\SteveCAD\\Mod\\SteveCAD\\SteveCADAgentCli.py",
                "open",
                "--path",
                "C:\\Models\\part.FCStd",
            ]
...
def test_cli_preserves_legacy_compatibility_base_url(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda: SimpleNamespace(
            ...
            load_endpoint=lambda: {
                ...
                "port": 443,
                "base_url": "https://gateway.example/stevecad-control",
                "fail_closed": False,
            },
            configured_port=lambda: 8766,
            load_token=lambda: "secret-token",
    ...
    assert opened[0].full_url == (
        "https://gateway.example/stevecad-control/v1/status"
    )
...
def test_cli_maps_semantic_menu_snapshot_and_independent_ui_click() -> None:
    ...
    screenshot = cli.build_parser().parse_args(
        ["screenshot", "--path", "C:\\Evidence\\stevecad.png"]
    )
