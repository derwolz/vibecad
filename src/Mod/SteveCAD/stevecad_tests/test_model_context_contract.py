...
from SteveCADProject import SteveCADConversationStore, _validated_conversation_turns
...
def _active_state(object_count: int = 10) -> dict:
    return {
        ...
        "modeling_surface": {
            ...
            "engine": "vibescript",
            "domain": "assemblies",
            "surface_id": "stevecad/surface/assembly/vibescript",
            "available": True,
        },
        "document": {
            "name": "Mechanism",
...
def test_provider_input_budget_reports_section_bytes_without_prompt_content() -> None:
    ...
    assert budget["schema"] == "stevecad-provider-input-budget-v1"
...
def test_first_prompt_context_json_includes_toplevel_aero() -> None:
    """STEVECAD_CONTEXT_JSON is the first-prompt path, not steering.
...
def test_oversized_selection_is_rejected_before_object_enumeration(
    ...
    from SteveCADCore import MAX_PROVIDER_SELECTION_ITEMS
...
def test_provider_context_does_not_copy_conversation_cache() -> None:
    ...
    assert not hasattr(SteveCADService, "provider_conversation_cache_snapshot")
...
def test_conversation_history_read_is_scoped_to_the_selected_thread(
    ...
    uncached_service = object.__new__(SteveCADService)
...
def test_unsaved_run_prompt_reaches_provider_and_includes_active_thread_once(
    ...
    assert context_ready["input_budget"]["schema"] == (
        "stevecad-provider-input-budget-v1"
    )
...
def test_explicit_inspected_image_transport_metadata_is_never_model_text() -> None:
    ...
    assert "_stevecad_image_attachment" not in visible
...
def test_oversized_tool_result_omits_whole_values_without_sampling() -> None:
    result = {
        ...
        "_stevecad_image_attachment": {"path": "/private/image.png"},
    }
    ...
    assert visible["stevecad_result_boundary"]["original_json_bytes"] > encoded_bytes
    assert "_stevecad_image_attachment" not in visible
...
def test_terminal_source_operation_never_omits_its_verdict_or_recovery() -> None:
    ...
    assert "_stevecad_value_omitted" not in json.dumps(terminal)
...
def test_concise_source_read_states_current_revision_directly() -> None:
    result = {
        ...
        "_stevecad_source_read_result": True,
    }
