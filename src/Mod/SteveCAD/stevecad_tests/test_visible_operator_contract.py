...
OPERATOR_SCRIPT = REPOSITORY_ROOT / "Invoke-SteveCAD-VisibleTour.ps1"
AGENT_CONTROL = (
    REPOSITORY_ROOT / "src" / "Mod" / "SteveCAD" / "SteveCADAgentControl.py"
)
...
def _receipt_fixture_script(receipt_path: Path) -> str:
    return (
        ...
        + _function_loader(
            ("New-SteveCADTourReceiptPayload", "Write-SteveCADTourReceipt")
        )
        + f"""
...
$Payload = New-SteveCADTourReceiptPayload `
    -ProcessId 4321 `
    -Executable 'C:\\SteveCAD\\package\\rattler-build\\.pixi\\envs\\default\\Library\\bin\\SteveCAD.exe' `
    -Targets $Targets `
    -ReceiptPath $ReceiptPath `
    -CompletedAtUtc '2026-08-29T17:45:02.0000000Z'
$Json = Write-SteveCADTourReceipt -Payload $Payload -ReceiptPath $ReceiptPath
...
def test_visible_tour_is_a_repo_local_one_click_entry_point() -> None:
    ...
    assert ".stevecad-dev\\agent" in source
    assert "127.0.0.1" in source
    assert "package\\rattler-build\\.pixi\\envs\\default" in source
    assert "SteveCAD.exe" in source
...
def test_visible_tour_native_calls_are_exactly_allowlisted_and_nonactivating() -> None:
    ...
    restore_start = source.index("function Restore-SteveCADWindowIfMinimized")
    ...
    assert restore_body.index("Get-SteveCADWindowBounds") < restore_body.index(
        "::ShowWindow("
    )
...
def test_visible_tour_has_a_plain_cyan_virtual_cursor() -> None:
    source = _script()
    assert "SteveCADVirtualCursor" in source
...
def test_visible_tour_discovers_live_menus_and_tabs_without_feature_assumptions() -> None:
    source = _script()
    assert "function Get-SteveCADDefaultTourTargets" in source
    assert "$Targets = @(Get-SteveCADDefaultTourTargets)" in source
...
def test_visible_tour_still_accepts_an_explicit_focused_sequence() -> None:
    ...
    assert "Resolve-SteveCADUiTarget" in source


def test_visible_tour_restores_only_the_exact_minimized_stevecad_window() -> None:
    source = _script()
...
def test_visible_tour_default_receipt_paths_are_unique_and_collision_resistant(
    ...
    helper_path.write_text(
        "$ErrorActionPreference = 'Stop'\n"
        + _function_loader(("New-SteveCADTourReceiptPath",))
        + f"""
$RepositoryRoot = {_powershell_literal(tmp_path)}
$First = New-SteveCADTourReceiptPath -RepositoryRoot $RepositoryRoot
$Second = New-SteveCADTourReceiptPath -RepositoryRoot $RepositoryRoot
...
""",
    ...
    expected_parent = (tmp_path / ".stevecad-dev" / "tours").resolve()
...
def test_visible_tour_receipt_schema_order_and_persisted_output_are_identical(
    ...
    assert payload["schema"] == "stevecad.visible-operator-receipt.v1"
    assert payload["ok"] is True
    assert payload["channel"] == "stevecad-visible-operator"
    assert payload["custom_cursor"] == "SteveCADVirtualCursor"
    ...
    assert payload["executable"] == (
        "C:\\SteveCAD\\package\\rattler-build\\.pixi\\envs\\default\\"
        "Library\\bin\\SteveCAD.exe"
    )
