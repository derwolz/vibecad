...
"""Guardrails for the user-facing SteveCAD product identity."""
...
class TestSteveCADNativePanelStartup(unittest.TestCase):
    ...
    def test_every_supported_workbench_creates_stevecad_panels(self) -> None:
        ...
        for workbench in self.WORKBENCHES:
            with self.subTest(workbench=workbench):
                ...
                panel_module = sys.modules.get("SteveCADGui")
                self.assertIsNotNone(
                    panel_module,
                    f"{workbench} did not initialize the shared SteveCAD GUI module",
                )
                self.assertIsNotNone(
                ...
                assistant = main_window.findChild(
                    QtWidgets.QDockWidget, "SteveCADAssistantPanel"
                )
                editor = main_window.findChild(
                    QtWidgets.QDockWidget, "SteveCADScriptedModelPanel"
                )
...
class TestSteveCADResponsiveAssistant(unittest.TestCase):
    ...
    def test_crash_recovery_is_non_modal_and_draft_autosave_is_debounced(
        ...
        root = SteveCADGui._build_panel_widget()
        try:
    ...
    def test_ctrl_enter_sends_while_plain_enter_edits(self) -> None:
        ...
        application = QtWidgets.QApplication.instance()
        self.assertIsNotNone(application)
        original_submit = SteveCADGui._run_prompt_from_panel
        submitted: list[str] = []
        root = None
        try:
            SteveCADGui._run_prompt_from_panel = lambda: submitted.append(
                prompt.toPlainText()
            )
            root = SteveCADGui._build_panel_widget()
            ...
            SteveCADGui._run_prompt_from_panel = original_submit
            if root is not None:
                root.close()
                root.deleteLater()
                application.processEvents()
    ...
    def test_transcript_turn_starts_outside_prior_markdown_list(self) -> None:
        ...
        SteveCADGui._append_transcript_block(
            output,
            SteveCADGui._transcript_block_html(
                "SteveCAD:\nFinished:\n\n- Added slots\n- Added fillets"
            ),
        )
        SteveCADGui._append_transcript_block(
            output,
            SteveCADGui._transcript_block_html("User:\nMake the slots longer"),
        )
    ...
    def test_narrow_composer_uses_distinct_icons_without_words(self) -> None:
        ...
        try:
            ...
            self.assertLess(
                composer.width(),
                SteveCADGui._COMPOSER_ICON_ONLY_BREAKPOINT,
            )
            buttons = {
            ...
            SteveCADGui._update_composer_button_presentation(
                composer,
                busy=False,
            )
            application.processEvents()
...
class TestSteveCADRibbonChrome(unittest.TestCase):
    ...
    def test_ribbon_replaces_workbench_chrome_without_removing_native_actions(
        ...
        ribbon = main_window.findChild(QtWidgets.QWidget, "SteveCADRibbon")
        toolbar = main_window.findChild(QtWidgets.QToolBar, "SteveCADRibbonToolBar")
        ...
        app_button = main_window.findChild(QtWidgets.QToolButton, "SteveCADAppButton")
...
def test_windows_installer_uses_stevecad_identity() -> None:
    ...
    assert '!define APP_NAME "SteveCAD"' in installer
    assert '!define APP_RUN "bin\\SteveCAD.exe"' in declarations
    assert '!define BIN_FREECAD "SteveCAD.exe"' in declarations
    assert '!define SETUP_ICON "icons\\SteveCAD.ico"' in declarations
...
def test_runtime_branding_resources_are_registered() -> None:
    ...
    assert 'Config()["ExeName"] = "SteveCAD"' in main_gui
    assert 'Config()["AppIcon"] = "stevecad"' in main_gui
    assert 'Config()["SplashScreen"] = "stevecadsplash"' in main_gui
    for asset in (
        "stevecad.svg",
        "stevecadabout.png",
        "stevecadaboutdev.png",
        "stevecadsplash.png",
        "stevecadsplash_2x.png",
    ):
        assert f"<file>{asset}</file>" in resources
        assert (ROOT / "src" / "Gui" / "Icons" / asset).is_file()


def test_every_runtime_entry_point_uses_only_the_stevecad_config_namespace() -> None:
    for relative_path in (
        ...
        assert 'Config()["ExeName"] = "SteveCAD"' in source
        assert 'Config()["ExeVendor"] = "SteveCAD"' in source
...
def test_fresh_gui_profiles_initialize_the_native_tree_before_main_window_construction() -> (
    ...
    defaults = main_gui.split("static void initializeSteveCADDockDefaults()", 1)[1]
    ...
    assert startup.index("initializeSteveCADDockDefaults();") < startup.index(
        "Gui::Application::initApplication();"
    )


def test_stevecad_docks_are_registered_before_native_window_restore() -> None:
    gui = _source("src/Mod/SteveCAD/SteveCADGui.py")
    init_gui = _source("src/Mod/SteveCAD/InitGui.py")
    ...
    assert "SteveCADGui.ensure_commands_registered()" in init_gui
...
def test_stevecad_docks_use_native_standard_workbench_declarations() -> None:
    ...
    assert '"SteveCADAssistantPanel"' in setup
    assert '"SteveCADScriptedModelPanel"' in setup
    assert '"SteveCADContextDebugPanel"' in setup
    assert "Gui::DockWindowOption::VisibleTabbed" in setup
    assert "Gui::DockWindowOption::HiddenTabbed" in setup
    context_debug_options = setup.rsplit('"SteveCADContextDebugPanel"', 1)[1].split(
        ");", 1
    )[0]
    ...
    for relative_path in ROOT.glob("src/Mod/*/InitGui.py"):
        if relative_path.parent.name == "SteveCAD":
            continue
        source = relative_path.read_text(encoding="utf-8")
        assert "register_ai_commands_for_workbench" not in source
        assert "import SteveCADGui" not in source


def test_stevecad_connects_one_global_workbench_activation_signal(monkeypatch) -> None:
    import SteveCADGui as panel
...
def test_stevecad_docks_do_not_run_a_manual_layout_restore(monkeypatch) -> None:
    import SteveCADGui as panel
...
def test_mcp_settings_have_a_dedicated_stevecad_preference_page() -> None:
    preferences = _source("src/Mod/SteveCAD/SteveCADPreferences.py")
    gui = _source("src/Mod/SteveCAD/SteveCADGui.py")
    main_page = preferences.split("class SteveCADPreferencesPage:", 1)[1].split(
        "class SteveCADMCPPreferencesPage:", 1
    )[0]
    ...
    assert 'setObjectName("SteveCADPrefMCPEnabled")' in mcp_page
    assert 'setObjectName("SteveCADPrefMCPEnabled")' not in main_page
    ...
    assert "SteveCADPreferences.SteveCADMCPPreferencesPage" in registration
...
def test_overlaid_dock_visibility_has_one_requested_state_owner() -> None:
    ...
    assert 'QStringLiteral("SteveCADViewportCanvas")' in main_window
    assert 'QStringLiteral("SteveCADModelBrowserHost")' in main_window
    assert 'QStringLiteral("SteveCADModelBrowserResizeHandle")' in main_window
    ...
    assert 'view->property("stevecadUsesModelBrowser")' in main_window
    ...
    assert "SteveCADModelBrowserHost" in tree
...
def test_windows_bundle_creates_branded_executable() -> None:
    ...
    launcher_source = _source("src/Main/SteveCADPortableLauncher.cpp")

    assert '"${copy_dir}/bin/SteveCAD.exe"' in bundle_script
    assert '[[ ! -x "${copy_dir}/bin/SteveCAD.exe" ]]' in bundle_script
    assert '"${copy_dir}/SteveCAD.exe"' in bundle_script
    assert "SteveCADPortableLauncher.exe" in bundle_script
    assert "SteveCADCmdPortableLauncher.exe" in bundle_script
    ...
    assert 'version_name="SteveCAD_${BUILD_TAG}' not in bundle_script
    assert 'rm -rf -- "${copy_dir}" "${version_name}" ".nsis_tmp"' in bundle_script
    assert "add_executable(SteveCADPortableLauncher WIN32" in main_cmake
    assert "add_executable(SteveCADCmdPortableLauncher" in main_cmake
    assert 'L"bin\\\\SteveCAD.exe"' in launcher_source
...
def test_every_release_bundle_keeps_and_executes_the_geometry_worker() -> None:
    ...
    local = _source("package/rattler-build/scripts/build_stevecad_local_release.sh")
    geometry = _source("src/Mod/SteveCAD/SteveCADGeometry.py")

    assert (
        'copy_matching_files "${conda_env}/Library/bin" '
        '"SteveCADGeometryWorker.exe" "${copy_dir}/bin"'
    ) in windows
    ...
    for release_script in (windows, linux, macos, local):
        assert "from SteveCADGeometry import runtime_execution_smoke" in release_script
...
def test_release_packages_share_canonical_artifact_basename() -> None:
    ...
    release_workflow = _source(".github/workflows/stevecad-release.yml")
    macos_workflow = _source(".github/workflows/stevecad-macos.yml")
    ...
    assert "package/rattler-build/osx/SteveCAD-*.dmg" in macos_workflow
    assert "package/rattler-build/osx/SteveCAD_*.dmg" not in macos_workflow


def test_update_ui_uses_the_stevecad_bar_and_keeps_preferences_for_settings() -> None:
    update_gui = _source("src/Mod/SteveCAD/SteveCADUpdateGui.py")
    preferences = update_gui.split("class SteveCADUpdatePreferencesPage", 1)[1].split(
        "class UpdateCenterDialog", 1
    )[0]

    assert '_COMMAND_NAME = "SteveCAD_CheckForUpdates"' in update_gui
    assert '_LEGACY_COMMAND_NAME = "SteveCAD_UpdateCenter"' in update_gui
    assert '_ICON = "view-refresh.svg"' in update_gui
    assert '"MenuText": "Check for Updates"' in update_gui
    assert 'action.setProperty("SteveCADCheckForUpdates", True)' in update_gui
    ...
    assert 'self.setWindowTitle("SteveCAD Updates")' in update_gui
    assert 'buttons.addButton("Download update"' in update_gui
    assert "show_check_for_updates(check_now=False)" in update_gui


def test_assistant_panel_uses_stevecad_product_name() -> None:
    panel_source = _source("src/Mod/SteveCAD/SteveCADGui.py")
    core_source = _source("src/Mod/SteveCAD/SteveCADCore.py")
    product_copy = panel_source + core_source

    for stale_copy in (
        "Create and save a FreeCAD document to enable SteveCAD.",
        "Save this FreeCAD document to enable SteveCAD.",
        "Looking at the current FreeCAD document...",
        "Summarize the current FreeCAD context.",
    ):
        assert stale_copy not in product_copy
    assert "Create and save a SteveCAD document to enable SteveCAD." in core_source
    assert "Looking at the current SteveCAD document..." in panel_source


def test_stevecad_ships_exactly_two_constrained_appearance_profiles() -> None:
...
def test_stevecad_does_not_expose_the_legacy_workbench_preferences_page() -> None:
...
def test_stevecad_removes_theme_and_preference_pack_escape_hatches() -> None:
...
def test_stevecad_ribbon_has_explicit_domains_and_legacy_fallback() -> None:
    ribbon = _source("src/Gui/SteveCADRibbon.cpp")
    startup = _source("src/Gui/StartupProcess.cpp")
    stevecad_gui_startup = _source("src/Mod/SteveCAD/InitGui.py")
    ...
    assert 'QStringLiteral("SteveCADAeroWorkspaceHost")' not in ribbon
    ...
    assert '("InspectionGui", "MeshPartGui", "PartGui")' in stevecad_gui_startup
    ...
    assert 'QStringLiteral("SteveCAD_OpenPreferences")' in ribbon
    assert 'QStringLiteral("SteveCAD_CheckForUpdates")' in ribbon
    assert "SteveCADRibbon::install(mainWindow);" in startup


def test_stevecad_migrates_its_obsolete_background_autoload_before_use() -> None:
    startup = _source("src/Gui/StartupProcess.cpp")
    activation = startup.split("void StartupPostProcess::activateWorkbench()", 1)[
        1
    ].split("void StartupPostProcess::setStyleSheet()", 1)[0]
    assert activation.index("migrateSteveCADBackgroundAutoload(wb);") < activation.index(
        "autoloadModules(wb);"
    )
    migration = startup.split(
        "void StartupPostProcess::migrateSteveCADBackgroundAutoload", 1
    )[1].split("void StartupPostProcess::autoloadModules", 1)[0]
    assert 'migrationKey = "SteveCADBackgroundAutoloadModules2026"' in migration
    assert 'general->RemoveASCII("BackgroundAutoloadModules");' in migration


def test_stevecad_bootstrap_repairs_only_stevecad_disabled_lists(monkeypatch) -> None:
    ...
    monkeypatch.setitem(
        sys.modules,
        "SteveCADAnalyzeStudyGui",
        SimpleNamespace(ensure_command_registered=lambda: None),
    )

    namespace = runpy.run_path(str(ROOT / "src/Mod/SteveCAD/InitGui.py"))
    ...
    assert namespace["_restore_stevecad_disabled_workbenches"]() is False
    ...
    assert namespace["_restore_stevecad_disabled_workbenches"]() is True
    assert preferences.disabled == "TestWorkbench,NoneWorkbench"


def test_stevecad_bootstrap_helpers_survive_freecad_exec_namespace(monkeypatch) -> None:
    ...
    init_gui = ROOT / "src/Mod/SteveCAD/InitGui.py"
...
def test_stevecad_bootstrap_isolates_optional_command_registration_failures(
    monkeypatch,
) -> None:
    ...
    monkeypatch.setitem(
        sys.modules,
        "SteveCADManufactureFollowUpGui",
        SimpleNamespace(ensure_command_registered=fail_follow_up),
    )
    monkeypatch.setitem(
        sys.modules,
        "SteveCADManufactureSimulationResultGui",
        SimpleNamespace(
    ...
    runpy.run_path(str(ROOT / "src/Mod/SteveCAD/InitGui.py"))
...
def test_setup_agent_control_invokes_local_stevecadgui_import(monkeypatch) -> None:
    """The deferred callback must import SteveCADGui in its own body.
    ...
    monkeypatch.delenv("STEVECAD_DEV_MODE", raising=False)
    ...
    monkeypatch.setenv("STEVECAD_DEV_MODE", "true")
    loader_locals["_setup_agent_control"]()
    assert started == [
    ...
    monkeypatch.setenv("STEVECAD_DEV_MODE", "1")
    loader_locals["_setup_agent_control"]()
    assert started == [
        {"document_thread_dispatch": dispatch},
        {"document_thread_dispatch": dispatch},
...
def test_stevecad_bootstrap_migrates_removed_bim_preferences(monkeypatch) -> None:
    class ParameterGroup:
    ...
    assert migration.values["SteveCADRemovedArchitectureWorkbench2026"] is True
    assert migration.values["SteveCADConsolidatedPartWorkbench2026"] is True
...
def test_stevecad_bootstrap_migrates_removed_openscad_preferences(monkeypatch) -> None:
    class ParameterGroup:
        def __init__(self, values=None) -> None:
            self.values = dict(values or {})
    ...
    assert migration.values["SteveCADRemovedOpenSCADWorkbench2026"] is True
    assert any(
