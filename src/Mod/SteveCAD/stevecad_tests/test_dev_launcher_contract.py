...
POWERSHELL = REPO_ROOT / "Launch-SteveCAD-Dev.ps1"
CMD = REPO_ROOT / "Launch-SteveCAD-Dev.cmd"
ONE_CLICK_CMD = REPO_ROOT / "RUN-STEVECAD-DEV.cmd"
INIT_GUI = REPO_ROOT / "src" / "Mod" / "SteveCAD" / "InitGui.py"
...
def test_windows_dev_launcher_uses_standalone_hash_provider_before_runtime_probe():
    ...
    hash_helper = "function Get-SteveCADSha256"
    runtime_probe = "function Test-SteveCADQtPlatformRuntime"
...
def test_windows_dev_launcher_is_repo_local_and_rebuilds_current_checkout():
    ...
    assert "pixi reinstall -e default stevecad" in script
    assert "& $Pixi reinstall -e default stevecad --frozen" in script
    assert "& $Pixi install -e default --frozen" in script
    assert "[switch]$SkipRebuild" in script
    assert r"Library\bin\SteveCAD.exe" in script
...
def test_windows_dev_launcher_recovers_an_incomplete_repo_local_environment():
    ...
    assert "$LaunchRuntime = Get-SteveCADLaunchRuntime" in script
    assert "if (-not $LaunchRuntime.Complete)" in script
    assert "pixi clean --build" in script
    assert "Recovering an incomplete repo-local SteveCAD development environment" in script


def test_windows_dev_launcher_requires_repo_local_qwindows_and_matching_qt_dlls():
    script = _text(POWERSHELL)

    assert "function Resolve-SteveCADQtLaunchRuntime" in script
    assert "function Get-SteveCADLaunchRuntime" in script
    for path in (
    ...
    assert "Get-SteveCADQtRuntimeIdentity" in script
...
def test_windows_dev_launcher_load_probes_qwindows_for_recovery_and_prelaunch():
    ...
    assert "function Test-SteveCADQtPlatformRuntime" in script
    ...
    assert "STEVECAD_EXPECTED_QWINDOWS" in script
    assert "GetModuleHandleW" in script
    assert "GetModuleFileNameW" in script
    assert "loaded_qwindows_path" in script
    assert script.count("Test-SteveCADQtPlatformRuntime -LaunchRuntime") >= 3
    initial_probe = "$InitialQtProbe = Test-SteveCADQtPlatformRuntime"
    recovery_gate = "if (-not $LaunchRuntime.Complete)"
    final_probe = "$FinalQtProbe = Test-SteveCADQtPlatformRuntime"
...
def test_windows_dev_launcher_streams_python_runtime_probe_over_stdin():
    ...
    runtime_probe_start = script.index("function Get-SteveCADPythonRuntimeProbe")
    runtime_probe_end = script.index("function Install-SteveCADPythonRuntime")
    runtime_probe = script[runtime_probe_start:runtime_probe_end]

    assert "$RuntimeProbe = Get-SteveCADPythonRuntimeProbe" in runtime_probe
    assert "$RuntimeProbe | & $PythonExecutable -" in runtime_probe
    assert "& $PythonExecutable -c" not in runtime_probe
    assert "& $PythonExecutable -c (Get-SteveCADPythonRuntimeProbe)" not in script


def test_windows_dev_launcher_rejects_qwindows_resolving_outside_env_root():
    script = _text(POWERSHELL)

    assert "function Test-SteveCADPathWithinRoot" in script
    assert "Test-SteveCADPathWithinRoot `" in script
    assert "resolved outside the checkout Pixi environment" in script
    qt_resolver = script[
        script.index("function Resolve-SteveCADQtLaunchRuntime") : script.index(
...
def test_windows_dev_launcher_refuses_incomplete_skip_rebuild_before_launch():
    ...
    refusal = (
        "SkipRebuild requested, but the repo-local SteveCAD launch runtime is incomplete"
    )
...
def test_windows_dev_launcher_exports_only_exact_repo_local_qt_plugin_paths():
    ...
    final_validation = (
        "$FinalLaunchRuntime = Get-SteveCADLaunchRuntime -EnvironmentRoot "
        "$ResolvedEnvRoot"
    )
...
def test_windows_dev_launcher_rehashes_executable_after_final_probe_before_launch():
    ...
    expected_hash = "$ExecutableSha256 = Get-SteveCADSha256 -Path $ResolvedExecutable"
    final_hash = (
        "$FinalExecutableSha256 = Get-SteveCADSha256 -Path "
...
def test_release_attestation_requires_clean_superproject_and_submodules():
    ...
    assert "function Assert-SteveCADReleaseCheckoutClean" in script
    ...
    pre_build = (
        ...
        'Assert-SteveCADReleaseCheckoutClean -RepositoryRoot $RepoRoot'
    )
    pre_receipt = (
...
def test_cold_build_safely_detaches_repo_local_pixi_staging_before_install():
    ...
    helper_start = script.index("function Move-SteveCADLocalBuildStagingAside")
    helper_end = script.index("function Resolve-SteveCADQtLaunchRuntime")
    helper = script[helper_start:helper_end]

    assert 'Join-Path $ResolvedPackageRoot ".pixi"' in helper
    assert 'Join-Path $PixiRoot "bld"' in helper
    assert "Test-SteveCADPathWithinRoot" in helper
    assert "[System.IO.FileAttributes]::ReparsePoint" in helper
    assert "resolved outside the repository package root" in helper
    assert "stevecad-build-staging-quarantine-" in helper
    ...
    detach = "Move-SteveCADLocalBuildStagingAside -PackageRoot $PackageRoot"
...
def test_windows_dev_launcher_sets_visible_identity_environment():
    ...
    assert '$env:STEVECAD_DEV_MODE = "1"' in script
    assert "$env:STEVECAD_DEV_SOURCE_SHA = $GitCommit" in script
    assert "$env:STEVECAD_DEV_SOURCE_TREE = $GitTree" in script
    assert "$env:STEVECAD_DEV_SOURCE_ROOT = $RepoRoot" in script
...
def test_windows_dev_launcher_writes_collision_safe_build_and_launch_attestations():
    ...
    assert "stevecad.dev-build-attestation.v1" in script
    assert "stevecad.dev-launch-attestation.v1" in script
    assert "function Write-SteveCADAttestation" in script
    assert "[System.IO.FileMode]::CreateNew" in script
    assert r'$AttestationRoot = Join-Path $RepoRoot ".stevecad-dev\attestations"' in script
...
def test_windows_dev_launcher_exports_exact_attestation_contract():
    ...
    assert "stevecad.dev-runtime-identity.v1" in script
...
def test_windows_dev_launcher_scopes_agent_control_to_this_checkout():
    ...
    assert r'$AgentHome = Join-Path $RepoRoot ".stevecad-dev\agent"' in script
    assert "$env:STEVECAD_AGENT_HOME = $AgentHome" in script
...
def test_windows_dev_launcher_waits_for_authenticated_agent_control_readiness():
    ...
    assert "function Wait-SteveCADAgentControl" in script
    ...
    assert '$Status.channel -eq "stevecad-agent-control"' in script
    ...
    assert "Wait-SteveCADAgentControl `" in script


def test_windows_dev_launcher_prepares_the_embedded_python_runtime():
    script = _text(POWERSHELL)

    assert "function Test-SteveCADPythonRuntime" in script
    assert "function Install-SteveCADPythonRuntime" in script
    assert r"src\Mod\SteveCAD\requirements.txt" in script
    assert r"src\Mod\SteveCADAero\requirements-aero.txt" in script
    ...
    assert "STEVECAD_RUNTIME_REQUIREMENTS" in script
...
def test_windows_dev_launcher_quarantines_python_user_site_packages():
    ...
    assert r'$PythonUserBase = Join-Path $RepoRoot ".stevecad-dev\python-user"' in script
...
def test_double_click_cmd_only_delegates_to_repo_root_powershell_launcher():
    ...
    assert '"%~dp0Launch-SteveCAD-Dev.ps1"' in script
...
def test_prominent_one_click_entry_point_delegates_to_the_canonical_launcher():
    ...
    assert 'cd /d "%~dp0"' in script
    assert 'call "%~dp0Launch-SteveCAD-Dev.cmd" %*' in script
    assert "Program Files" not in script
...
def test_gui_bootstrap_consumes_dev_identity_without_affecting_normal_launches():
    ...
    assert 'os.environ.get("STEVECAD_DEV_MODE")' in script
    assert 'os.environ.get("STEVECAD_DEV_SOURCE_SHA")' in script
    assert "development_runtime_identity" in script
    assert "SteveCADDevelopmentIdentity" in script
    assert "SteveCAD DEV" in script
