...
in-app SteveCAD Assistant, so Grok / ChatGPT / OpenAI / Anthropic can keep
...
Callers authenticate with a bearer token that SteveCAD writes to a private file
...
AGENT_PORT_ENV = "STEVECAD_AGENT_PORT"
AGENT_HOME_ENV = "STEVECAD_AGENT_HOME"
TOKEN_FILENAME = "token"
ENDPOINT_FILENAME = "endpoint.json"
AGENT_BRIEF_FILENAME = "AGENTS.md"
GROK_BOT_CMD_ENV = "STEVECAD_GROK_BOT_CMD"
TOKEN_BYTES = 32
MAX_BODY_BYTES = 1_048_576
DEV_MODE_ENV = "STEVECAD_DEV_MODE"
DEV_SOURCE_SHA_ENV = "STEVECAD_DEV_SOURCE_SHA"
DEV_SOURCE_TREE_ENV = "STEVECAD_DEV_SOURCE_TREE"
DEV_SOURCE_ROOT_ENV = "STEVECAD_DEV_SOURCE_ROOT"
DEV_ATTESTATION_REQUIRED_ENV = "STEVECAD_DEV_ATTESTATION_REQUIRED"
DEV_BUILD_ATTESTATION_ENV = "STEVECAD_DEV_BUILD_ATTESTATION"
DEV_BUILD_ATTESTATION_SHA256_ENV = "STEVECAD_DEV_BUILD_ATTESTATION_SHA256"
DEV_LAUNCH_ATTESTATION_ENV = "STEVECAD_DEV_LAUNCH_ATTESTATION"
DEV_LAUNCH_ATTESTATION_SHA256_ENV = "STEVECAD_DEV_LAUNCH_ATTESTATION_SHA256"
BUILD_ATTESTATION_SCHEMA = "stevecad.dev-build-attestation.v1"
LAUNCH_ATTESTATION_SCHEMA = "stevecad.dev-launch-attestation.v1"
RUNTIME_IDENTITY_SCHEMA = "stevecad.dev-runtime-identity.v1"
OPERATION_TRACKING_SCHEMA = "stevecad.dev-operation-tracking.v1"
...
_ATTESTED_RUNTIME_MODULES = (
    "InitGui.py",
    "SteveCADAgentControl.py",
    "SteveCADAgentCli.py",
    "SteveCADGui.py",
)
_ATTESTED_SOURCE_ONLY_MODULES = (
    "Invoke-SteveCAD-VisibleTour.ps1",
    "Launch-SteveCAD-Dev.ps1",
)
COMMANDS = (
    "status",
    "documents",
...
def _current_qwindows_module_path() -> Path:
    """Return the qwindows.dll loaded by this exact SteveCAD GUI process."""
    ...
    if not module:
        raise RuntimeError(
            "The current SteveCAD process has not loaded the qwindows.dll platform plugin."
        )
    ...
    return _canonical_path(
        Path(buffer.value), kind="qwindows.dll loaded by the current SteveCAD process"
    )
...
def _actual_runtime_module_paths() -> dict[str, Path]:
    ...
    gui_module = sys.modules.get("SteveCADGui")
    gui_path = Path(str(getattr(gui_module, "__file__", "") or ""))
    if not gui_path.is_file():
        gui_path = module_root / "SteveCADGui.py"
    return {
        "InitGui.py": _canonical_path(init_path, kind="InitGui.py"),
        "SteveCADAgentControl.py": _canonical_path(
            Path(__file__), kind="SteveCADAgentControl.py"
        ),
        "SteveCADAgentCli.py": _canonical_path(
            module_root / "SteveCADAgentCli.py", kind="SteveCADAgentCli.py"
        ),
        "SteveCADGui.py": _canonical_path(gui_path, kind="SteveCADGui.py"),
    }


def _expected_source_paths(repository_root: Path) -> dict[str, Path]:
    module_root = repository_root / "src" / "Mod" / "SteveCAD"
    return {
        "InitGui.py": module_root / "InitGui.py",
        "SteveCADAgentControl.py": module_root / "SteveCADAgentControl.py",
        "SteveCADAgentCli.py": module_root / "SteveCADAgentCli.py",
        "SteveCADGui.py": module_root / "SteveCADGui.py",
        "Invoke-SteveCAD-VisibleTour.ps1": (
            repository_root / "Invoke-SteveCAD-VisibleTour.ps1"
        ),
        "Launch-SteveCAD-Dev.ps1": repository_root / "Launch-SteveCAD-Dev.ps1",
    }
...
def _validated_qt_attestations(
    *,
    ...
    if not _same_path(process_qwindows, qwindows):
        raise RuntimeError(
            "The current SteveCAD process did not load the exact attested qwindows.dll."
        )
    process_qwindows_hash = _sha256_file(process_qwindows)
    if not secrets.compare_digest(process_qwindows_hash, qwindows_hash):
        raise RuntimeError(
            "The qwindows.dll loaded by the current SteveCAD process changed after attestation."
        )
...
def development_runtime_identity() -> dict[str, Any] | None:
    ...
    if str(os.environ.get(DEV_MODE_ENV) or "").strip() != "1":
        raise RuntimeError("Development attestations are valid only in STEVECAD_DEV_MODE=1.")
    ...
    if build_path.parent.name != "attestations" or build_path.parent.parent.name != ".stevecad-dev":
...
def agent_home() -> Path:
    ...
    if sys.platform == "win32":
        ...
        return root / "SteveCAD" / "Agent"
    if sys.platform == "darwin":
        return Path.home() / "Library" / "Application Support" / "SteveCAD" / "Agent"
...
def _restrict_path(path: Path, *, directory: bool) -> None:
    ...
    if required and sys.platform == "win32":
        try:
            ...
            if required:
                raise RuntimeError(
                    f"SteveCAD development mode could not enforce a current-user-only ACL on {path}: {exc}"
                ) from exc
    try:
        os.chmod(path, mode)
    except OSError as exc:
        if required:
            raise RuntimeError(
                f"SteveCAD development mode could not restrict {path}: {exc}"
            ) from exc
...
def _prepare_agent_home() -> Path:
    ...
    if _development_mode_enabled():
        ...
        if not source_root_value:
            raise RuntimeError(
                "SteveCAD development mode requires a checkout-scoped agent home and source root."
            )
        source_root = Path(source_root_value).expanduser()
        if not source_root.is_absolute():
            raise RuntimeError(
                "SteveCAD development mode requires an absolute checkout-scoped source root."
            )
        expected_home = (source_root.resolve() / ".stevecad-dev" / "agent").resolve()
        if not _same_path(home, expected_home):
            raise RuntimeError(
                "SteveCAD development mode requires the exact checkout-scoped "
...
def _server_identity_fields() -> dict[str, Any]:
    return {
        ...
        "fail_closed": bool(getattr(_server, "stevecad_fail_closed", False)),
    }
...
_AGENT_BRIEF_TEMPLATE = """# SteveCAD control brief for Grok Bot

SteveCAD is running on this machine and exposes an authenticated control
...
exact semantic SteveCAD targets without taking over the human's physical cursor.
...
| GET/POST | `/v1/screenshot` | optional `{{"path":"...png","overwrite":false}}` | Capture the visible SteveCAD window |
...
`run` executes Python in the SteveCAD process with `App`/`FreeCAD` (and
...
Open, save-as, and authorized `run` operations use the SteveCAD process's file
authority.
...
- Never type passwords or OAuth codes. Sign-in stays in SteveCAD Preferences.
- Do not enable MCP; it disables the in-app Assistant.
"""


def write_agent_brief(*, host: str = AGENT_HOST, port: int | None = None) -> Path:
    """Write an AGENTS.md brief telling a local agent how to drive SteveCAD.
...
def detect_grok_bot_command(explicit: str = "") -> str | None:
    ...
    Resolution order: an explicit path/command, the ``STEVECAD_GROK_BOT_CMD``
    ...
    *,
    ...
    if not _document_operation_gate.acquire(blocking=False):
        return failure(
            ...
            "Another SteveCAD document operation is still in progress; retry after it completes.",
            stage="precondition",
        )
    try:
        dispatch = _document_thread_dispatch
        if not callable(dispatch):
            ...
            return failure(
                ...
                "The SteveCAD GUI document-thread dispatcher is unavailable; no document state was accessed.",
                stage="precondition",
            )
...
def _execute_document_operation(operation: Callable[[], Any]) -> Any:
    ...
    if not callable(restoring):
        return failure(
            ...
            "SteveCAD cannot verify the native document-restore state; no document state was accessed.",
            stage="precondition",
        )
    try:
        if bool(restoring()):
...
def _partial_document_save_failure(document: Any) -> dict[str, Any] | None:
    ...
    try:
        ...
        return failure(
            ...
            "SteveCAD could not verify whether the document is partially loaded; refusing to save it.",
            stage="precondition",
        )
...
def _auth_snapshot(provider: str) -> dict[str, Any]:
    try:
        from SteveCADAuth import resolve_auth_state
...
def _grok_account_snapshot() -> dict[str, Any]:
    try:
        from SteveCADGrokAuth import cached_account
...
def report_status() -> dict[str, Any]:
    ...
    return {
        "ok": True,
        "channel": "stevecad-agent-control",
        **_server_identity_fields(),
        ...
        "oauth_note": (
            ...
            "a SteveCAD-specific OAuth app; SteveCAD reuses the official Grok CLI "
...
def open_document(path: str) -> dict[str, Any]:
    ...
    if document is None:
        return failure(
            ...
            f"SteveCAD could not open {candidate}.",
            stage="native_call",
        )
    App.setActiveDocument(str(document.Name))
    return {
...
def save_document(name: str = "") -> dict[str, Any]:
    ...
    if outcome is False or not path.is_file():
        return failure(
            "DOCUMENT_SAVE_FAILED",
            f"SteveCAD did not produce the expected document file at {path}.",
            stage="native_call",
        )
    ...
    if summary["modified"]:
        return failure(
            ...
            "SteveCAD saved the file but the document still reports unsaved changes.",
            stage="postcondition",
            saved=summary,
        )
    return {
...
def save_document_as(
    ...
    *,
    ...
    if outcome is False or not candidate.is_file():
        return failure(
            "DOCUMENT_SAVE_AS_FAILED",
            f"SteveCAD did not produce the expected document file at {candidate}.",
            stage="native_call",
        )
    try:
    ...
    if actual != candidate:
        return failure(
            ...
            "SteveCAD saved a file but did not associate the document with the requested path.",
            stage="postcondition",
...
def ui_ribbon_snapshot() -> dict[str, Any]:
    ...
    if gui is None or not bool(getattr(gui, "GuiUp", True)):
        return failure(
            "GUI_REQUIRED",
            "Ribbon geometry requires the running SteveCAD GUI.",
        )
    try:
        ...
        if tabs is None:
            return failure(
                ...
                "The SteveCADRibbonTabs semantic target is unavailable.",
            )
        ...
        return {
            ...
            "object_name": str(tabs.objectName() or "SteveCADRibbonTabs"),
...
def ui_menu_snapshot() -> dict[str, Any]:
    ...
    if gui is None or not bool(getattr(gui, "GuiUp", True)):
        return failure(
            "GUI_REQUIRED",
            "Menu geometry requires the running SteveCAD GUI.",
        )
    try:
        main_window = gui.getMainWindow()
        ...
        if menu_bar is None:
            return failure(
                ...
                "The SteveCAD top-level menu bar is unavailable.",
            )
        items: list[dict[str, Any]] = []
        ...
        return {
            ...
            "process_id": os.getpid(),
            "window_handle": int(main_window.winId()),
            "object_name": str(menu_bar.objectName() or "SteveCADMenuBar"),
...
def ui_click_target(
    ...
    *,
    ...
    if required_pid and required_pid != os.getpid():
        return failure(
            "UI_PROCESS_MISMATCH",
            f"Expected SteveCAD PID {required_pid}, but this GUI is PID {os.getpid()}.",
            stage="precondition",
        )
    try:
    ...
    if gui is None or not bool(getattr(gui, "GuiUp", True)):
        return failure(
            "GUI_REQUIRED",
            "UI clicking requires the running SteveCAD GUI.",
        )
    try:
        ...
        if main_window is None:
            return failure(
                ...
                "The SteveCAD main window is unavailable.",
            )
        ...
        if target_kind == "ribbon":
            ...
            widget = main_window.findChild(QtWidgets.QTabBar, "SteveCADRibbonTabs")
            if widget is None or not bool(widget.isVisible()):
                return failure(
                    "RIBBON_TABS_UNAVAILABLE",
                    "The visible SteveCADRibbonTabs semantic target is unavailable.",
                )
            matches = [
                index
            ...
            if widget is None or not bool(widget.isVisible()):
                return failure(
                    "MENU_BAR_UNAVAILABLE",
                    "The visible SteveCAD top-level menu bar is unavailable.",
                )
...
def _resolve_screenshot_path(
    ...
    if not text:
        ...
        return (directory / f"stevecad-window-{timestamp}.png").resolve(), None
...
def capture_screenshot(
    ...
    *,
    ...
    """Capture the visible SteveCAD main window to a native PNG file."""
    ...
    if gui is None or not bool(getattr(gui, "GuiUp", True)):
        return failure(
            "GUI_REQUIRED",
            "Screenshot capture requires the running SteveCAD GUI.",
        )
    try:
        main_window = gui.getMainWindow()
        if main_window is None:
        ...
        if callable(is_visible) and not bool(is_visible()):
            return failure(
                ...
                "The SteveCAD main window is not visible.",
                stage="precondition",
            )
        ...
        if width <= 0 or height <= 0:
            return failure(
                "SCREENSHOT_EMPTY",
                "SteveCAD returned an empty main-window image.",
                stage="postcondition",
            )
        if not bool(pixmap.save(str(candidate), "PNG")):
            return failure(
                "SCREENSHOT_SAVE_FAILED",
                f"Qt could not save the SteveCAD screenshot at {candidate}.",
                stage="native_call",
            )
    except Exception as exc:
        return failure(
...
def aero_command(arguments: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if operation == "context":
        card = SteveCADAero.flight_card(document) if document is not None else {"ok": False}
        return {
            "ok": True,
    ...
    runners = {
        "analyze": lambda: SteveCADAero.run_analyze(document, repair=False),
        "section": lambda: SteveCADAero.run_section(document),
        "vlm": lambda: SteveCADAero.run_vlm(document),
        "export_jsbsim": lambda: SteveCADAero.export_jsbsim(document),
        "report": lambda: SteveCADAero.write_last_report(document),
        "propose_repairs": lambda: SteveCADAero.propose_repairs(document),
        "apply_repairs": lambda: SteveCADAero.apply_repairs(document),
        "reject_repairs": lambda: SteveCADAero.reject_repairs(document),
        "flight_card": lambda: SteveCADAero.flight_card(document),
    }
...
def show_preferences() -> dict[str, Any]:
    ...
    if not callable(show):
        return failure(
            "GUI_REQUIRED",
            "Showing Preferences requires the running SteveCAD GUI. "
            "Start SteveCAD.exe and call the loopback API, or open "
            "Edit → Preferences → SteveCAD yourself.",
            stage="precondition",
        )
    show("SteveCAD", "SteveCAD")
    return {"ok": True, "opened": "SteveCAD"}
...
def _server_accepts_client(server: Any, client_host: str) -> bool:
    ...
    if not bool(getattr(server, "stevecad_fail_closed", False)):
        return True
    ...
    def _handle(self, method: str) -> None:
        ...
        try:
            ...
            status, payload = handle_http_request(
                ...
                body,
                fail_closed=bool(
                    getattr(self.server, "stevecad_fail_closed", False)
                ),
                server_instance_id=getattr(
                    self.server,
                    "stevecad_server_instance_id",
                    None,
                ),
                server_identity=getattr(
                    self.server,
                    "stevecad_server_identity",
                    None,
                ),
            )
        except ValueError as exc:
...
def server_is_fail_closed() -> bool:
    ...
    with _server_lock:
        return bool(getattr(_server, "stevecad_fail_closed", False))
...
def _ensure_server_started(
    *,
    ...
    if fail_closed and host != AGENT_HOST:
        raise RuntimeError("SteveCAD agent control may bind only to 127.0.0.1.")
    with _server_lock:
        if _server is not None and _bound_port:
            running_fail_closed = bool(
                getattr(_server, "stevecad_fail_closed", False)
            )
            if fail_closed and not running_fail_closed:
                raise RuntimeError(
                    "SteveCAD agent control is already running in compatibility mode; "
            ...
            if document_thread_dispatch is not None:
                if (fail_closed or running_fail_closed) and not callable(
                    ...
                    raise RuntimeError(
                        "SteveCAD agent control requires a callable document-thread dispatcher."
                    )
                if (
                    running_fail_closed
                    and not fail_closed
        ...
        if fail_closed and not callable(document_thread_dispatch):
            raise RuntimeError(
                "SteveCAD agent control requires the GUI document-thread dispatcher before startup."
            )
        if document_thread_dispatch is not None:
            _document_thread_dispatch = document_thread_dispatch
        ...
        if listener is None:
            raise RuntimeError(
                f"SteveCAD agent control could not bind {host}:{requested}: {last_error}"
            )
        try:
            ...
            setattr(server, "stevecad_fail_closed", bool(fail_closed))
            setattr(server, "stevecad_server_instance_id", _server_instance_id)
            setattr(
                server,
                "stevecad_server_identity",
...
def shutdown_server(*, wait: bool = False) -> None:
    ...
    with _server_lock:
        ...
        was_fail_closed = bool(
            getattr(server, "stevecad_fail_closed", False)
        )
        _server = None
        _server_thread = None
        if was_fail_closed:
