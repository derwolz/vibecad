...
"""Authentication state helpers for SteveCAD providers.
...
SteveCAD never reads, copies, or stores those OAuth tokens. Grok / X sign-in
uses xAI OAuth and stores tokens in a private SteveCAD Grok credential
...
KEYRING_SERVICE = "FreeCAD SteveCAD"
...
def delete_keyring_key(provider: str = DEFAULT_PROVIDER) -> dict[str, str | bool]:
    ...
    if not spec.uses_api_key:
        return {
            "deleted": False,
            "error": f"{spec.display_name} has no SteveCAD API key to delete.",
        }
    keyring = _keyring_module()
    if keyring is None:
...
def resolve_auth_credential(
    ...
    if spec.auth_kind == "xai_oauth":
        try:
            from SteveCADGrokAuth import resolve_access_token
...
def resolve_auth_state(
    ...
    if spec.auth_kind == "xai_oauth":
        try:
            from SteveCADGrokAuth import resolve_tokens
    ...
    if not spec.uses_api_key:
        try:
            from SteveCADCodex import cached_account, runtime_health
...
def validate_configured_auth(
    *,
    ...
    if spec.auth_kind == "xai_oauth":
        try:
            from SteveCADGrokAuth import list_models, resolve_tokens
...
def list_provider_models(
    ...
    *,
    ...
    if spec.auth_kind == "xai_oauth":
        try:
            from SteveCADGrokAuth import list_models as list_grok_models
