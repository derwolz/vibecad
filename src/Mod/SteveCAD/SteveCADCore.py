...
"""Core SteveCAD context and read-only FreeCAD tools."""
...
from SteveCADIntentMemory import (
...
from SteveCADNativeAnalyzeContext import AnalyzeContextCoordinator
from SteveCADNativeDrawingContext import DrawingSourceCatalogCoordinator
from SteveCADNativeState import NativeDocumentStateStore, is_structural_property
...
from SteveCADProject import (
    AUTHORING_MODE_META_KEY,
    DEFAULT_CONVERSATION_TITLE,
    NATIVE_AUTHORITY_META_KEY,
    SteveCADConversationStore,
    SteveCADProjectStore,
...
class SteveCADService:
    ...
    def __init__(self, dotenv_path: Path | None = None) -> None:
        ...
        self._project_store = SteveCADProjectStore(self._local_session_id)
    ...
    def select_modeling_engine(self, mode: str) -> dict[str, str]:
        ...
        from SteveCADAuthoringMode import normalize_authoring_mode
    ...
    def _write_active_authoring_metadata(self, mode: str) -> None:
        document = self._active_document()
        if document is None:
            raise RuntimeError("SteveCAD has no active document authority to persist.")
        metadata = dict(getattr(document, "Meta", {}) or {})
    ...
    def begin_document_change_batch(
        ...
        *,
        ...
        from SteveCADDocumentChangeBatch import begin_document_change_batch
    ...
    def end_document_change_batch(
        ...
        *,
        ...
        from SteveCADDocumentChangeBatch import end_document_change_batch
    ...
    def native_active_snapshot(self) -> dict[str, Any]:
        ...
        from SteveCADModelingSurface import provider_engine_for_ribbon
    ...
    def begin_native_analyze_context_request(self) -> dict[str, Any] | None:
        ...
        if document is None:
            return None
        from SteveCADModelingSurface import provider_engine_for_ribbon
        from SteveCADNativeAnalyzeSnapshot import (
            begin_analyze_snapshot_capture,
            capture_analyze_clipping,
            capture_analyze_snapshot_batch,
        )
        from SteveCADNativeGeometrySources import drawing_analysis_artifact_names
        from SteveCADNativeSnapshot import capture_active_snapshot_base
        from SteveCADRibbonSurface import read_active_ribbon_surface
    ...
    def _validate_native_analyze_context_request(
        ...
        from SteveCADModelingSurface import provider_engine_for_ribbon
        from SteveCADNativeAnalyzeContext import AnalyzeContextStale
        from SteveCADRibbonSurface import read_active_ribbon_surface
    ...
    def capture_native_analyze_context_batch(
        ...
        from SteveCADNativeAnalyzeSnapshot import capture_analyze_snapshot_batch
    ...
    def capture_native_analyze_context_clipping(
        ...
        from SteveCADNativeAnalyzeSnapshot import capture_analyze_clipping
    ...
    def _validate_native_drawing_context_request(
        ...
        from SteveCADModelingSurface import provider_engine_for_ribbon
        from SteveCADNativeDrawingContext import DrawingContextStale
        from SteveCADRibbonSurface import read_active_ribbon_surface
    ...
    def finish_native_drawing_context_request(
        ...
        from SteveCADNativeDrawingSnapshot import build_drawing_snapshot
        from SteveCADNativeSnapshot import complete_active_snapshot
    ...
    def document_persistence_state(self) -> dict[str, Any]:
        ...
        if doc is None:
            return {
                ...
                "document": None,
                "file_path": None,
                "message": "Create and save a SteveCAD document to enable SteveCAD.",
            }
        ...
        if not file_path:
            return {
                ...
                "file_path": None,
                "message": "Save this SteveCAD document to enable SteveCAD.",
            }
        return {
            "enabled": True,
            "reason": "ready",
    ...
    def assistant_document_state(self) -> dict[str, Any]:
        ...
        if doc is None:
            return {
                ...
                "file_path": None,
                "saved": False,
                "message": "Create or open a document to use SteveCAD.",
            }
        uid = str(getattr(doc, "Uid", "") or "").strip()
        if not uid:
            return {
    ...
    def _object_summary(obj: Any) -> dict[str, Any]:
        ...
        role = str(getattr(obj, "SteveCADScriptedRole", "") or "")
        if role:
            summary["scripted_role"] = role
            summary["scripted_engine"] = str(
                getattr(obj, "SteveCADScriptedEngine", "") or ""
            )
            summary["scripted_model_id"] = str(
                getattr(obj, "SteveCADScriptedModelId", "") or ""
            )
            output_key = str(
                getattr(obj, "SteveCADScriptedOutputKey", "") or ""
            )
            if output_key:
    ...
    def _is_private_scripted_object(obj: Any) -> bool:
        return (
            str(getattr(obj, "SteveCADScriptedRole", "") or "")
    ...
    def _reference_artifact_dir(self) -> Path:
        ...
        Project roots always live under the central SteveCAD data dir, so
        references are never written next to the CAD file. Without a project
        context the default path still lands inside ``stevecad_data_dir()``.
        """
        try:
        ...
        return stevecad_data_dir() / "references"
    ...
    def _load_reference_images_for_active_project(self) -> None:
        try:
            ...
            path = stevecad_data_dir() / "references.json"
        key = str(path)
    ...
    def _write_reference_images(self) -> None:
        try:
            ...
            payload = {
                "format": "SteveCAD reference images",
                "version": 1,
            ...
            raise RuntimeError(
                f"SteveCAD references could not be written: {exc}"
            ) from exc
    ...
    def _geometry_summary(
        ...
        item = {
            ...
            "construction": SteveCADService._geometry_construction_state(
                geometry,
                index,
                sketch,
            ),
        ...
        shape_diagnostics = SteveCADService._geometry_shape_diagnostics(geometry)
    ...
    def _geometry_shape_diagnostics(geometry: Any) -> dict[str, Any]:
        ...
        result: dict[str, Any] = {
            "actual_bounds": SteveCADService._curve_bound_box_summary(shape.BoundBox),
        }
    ...
    def _sketch_junction_diagnostics(
        ...
        for geometry in geometry_summaries:
            ...
            for role in ("start", "end"):
                ...
                key = SteveCADService._rounded_endpoint_key(endpoint["point"])
        ...
        for endpoints in endpoint_groups.values():
            ...
            for first, second in itertools.combinations(endpoints, 2):
                ...
                first_tangent = SteveCADService._outward_endpoint_tangent(first)
                second_tangent = SteveCADService._outward_endpoint_tangent(second)
    ...
    def _sketch_native_geometry_diagnostics(
        ...
        if summaries is None:
            summaries = [
                SteveCADService._geometry_summary(item, index, sketch)
                for index, item in enumerate(geometry)
            ]
    ...
    def _assembly_component_objects(self, assembly: Any) -> list[Any]:
        ...
        for child in children:
            ...
            if id(child) in linked_target_ids:
                ...
                continue
            output_type = str(
                getattr(child, "SteveCADVibeScriptOutputType", "") or ""
            ).strip()
    ...
    def _point_artifact_project_root(
        ...
        if not root:
            raise RuntimeError(
                "Save or initialize the active SteveCAD project before managing "
                "point artifacts."
            )
        return root
    ...
    def approve_point_artifact(
        ...
        *,
        ...
        from SteveCADPointArtifacts import approve_point_artifact
    ...
    def remove_point_artifact(
        ...
        *,
        ...
        from SteveCADPointArtifacts import remove_point_artifact
    ...
    def _conversation_scope(self) -> dict[str, Any]:
        ...
        per-document project folder under the central SteveCAD data dir —
    ...
    def complete_conversation_history_read(
        ...
        store = SteveCADConversationStore(project_root)
    ...
    def conversation_snapshot_for_save(self, doc: Any) -> dict[str, Any]:
        ...
        store: SteveCADConversationStore | None = None
        if (
            self._conversation_cache_document_uid == document_uid
            ...
            if cached_directory.is_dir():
                store = SteveCADConversationStore(cached_directory.parent)
        if store is None:
            current_file = str(getattr(doc, "FileName", "") or "").strip()
            if current_file:
                current_root = project_root_for_document_file(current_file)
                current_store = SteveCADConversationStore(current_root)
                if (
    ...
    def prepare_session_recovery(
        ...
        *,
        ...
        if not project_root or not document_uid:
            raise RuntimeError("Open a CAD document before saving SteveCAD recovery.")
    ...
    def persist_prepared_session_recovery(
        ...
        from SteveCADSessionRecovery import SessionRecoveryStore

        project_root = str(prepared.get("project_root") or "").strip()
        if not project_root:
            raise RuntimeError("Prepared SteveCAD recovery has no project root.")
    ...
    def relocate_conversation_store_for_document_file(
        ...
        if not source:
            raise RuntimeError(
                "No SteveCAD conversation store was captured before save."
            )
        result = SteveCADConversationStore.relocate_directory(
            source,
    ...
    def discard_temporary_project_root(project_root: str | Path) -> bool:
        ...
        projects = (stevecad_data_dir() / "projects").expanduser().resolve()
        if root.parent != projects or not root.name.startswith("unsaved-"):
            raise RuntimeError(
                "Refusing to discard a project folder outside SteveCAD temporary scope."
            )
        if not root.exists():
            return False
        shutil.rmtree(root)
    ...
    def relocate_temporary_project_artifacts_for_document_file(
        ...
        if (
            ...
            raise RuntimeError("SteveCAD project relocation escaped its artifact scope.")
        ...
        def move_without_overwrite(source_path: Path, target_path: Path) -> None:
            ...
            if source_path.is_file() and target_path.is_file():
                if source_path.read_bytes() != target_path.read_bytes():
                    raise RuntimeError(
                        "Cannot merge SteveCAD project artifacts because "
                        f"{target_path.name!r} differs at the destination."
                    )
                source_path.unlink()
                return
            raise RuntimeError(
                "Cannot merge SteveCAD project artifacts with incompatible paths: "
    ...
    def persist_prepared_conversation_turn(prepared: dict[str, Any]) -> dict[str, Any]:
        ...
        with _CONVERSATION_WRITE_LOCK:
            ...
            if SteveCADService._conversation_has_equivalent_turn(conversation, entry):
    ...
    def recompute_diagnostics(self) -> dict[str, Any]:
        from SteveCADTransactions import recompute_diagnostic_summary
    ...
    def provider_tool_surface(self, workbench: str | None = None) -> dict[str, Any]:
        from SteveCADSession import provider_tool_schemas
    ...
    def is_provider_tool_available(
        ...
        from SteveCADSession import is_provider_safe_tool
    ...
    def _waypoint_summary(waypoint: Any, index: int) -> dict[str, Any]:
        ...
        if placement is not None:
            item["placement"] = SteveCADService._placement_summary(placement)
        return item
    ...
    def _part_objects(self) -> list[Any]:
        ...
        return [
            ...
            and (
                ...
                or str(getattr(obj, "SteveCADScriptedRole", "") or "")
                == "publication"
            )
        ]
    ...
    def context_summary(self) -> dict[str, Any]:
        ...
        context = {
            ...
            "workbench": self.active_workbench_name(),
            "stevecad_project": self.project_context(),
...
_service: SteveCADService | None = None


def get_service() -> SteveCADService:
    global _service
    if _service is None:
        _service = SteveCADService()
    return _service
