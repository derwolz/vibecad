...
from SteveCADNativeDrawingDetail import PreparedDetailView, validate_prepared_detail_view
...
DRAWING_DETAIL_PROTOCOL = "stevecad-native-drawing-detail-v1"
...
def create_detail_workspace() -> DrawingDetailWorkspace:
    temporary = tempfile.TemporaryDirectory(prefix="stevecad-native-drawing-detail-")
    root = Path(temporary.name).resolve()
    try:
        os.chmod(root, 0o700)
        child = freeze_regular_file(
            Path(__file__).with_name("SteveCADNativeDrawingDetailChild.py").resolve(),
            maximum=1024 * 1024,
        )
