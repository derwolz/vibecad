...
"""SteveCAD provider session orchestration.
...
from SteveCADCore import SteveCADService, get_service
...
from SteveCADIntentMemoryCompiler import compile_intent_memory_update
...
class _VibeScriptOperationManager:
    ...
    def start(
        ...
        threading.Thread(
            ...
            name=f"SteveCAD-VibeScript-{operation_id}",
            daemon=True,
        ).start()
        return response
...
def _vibescript_operation_manager(
    service: SteveCADService,
) -> _VibeScriptOperationManager:
    manager = getattr(service, "_stevecad_vibescript_operations", None)
    if not isinstance(manager, _VibeScriptOperationManager):
        manager = _VibeScriptOperationManager()
        try:
            setattr(service, "_stevecad_vibescript_operations", manager)
        except (AttributeError, TypeError):
            # Lightweight immutable test doubles do not retain cross-runner
            # state; the real SteveCAD service does.
            pass
    return manager


@dataclass(frozen=True)
class SteveCADResponse:
    provider: str
    final_output: str
...
def _document_recompute_state(service: SteveCADService) -> dict[str, Any]:
...
def _active_document_exists(service: SteveCADService) -> bool:
...
def _current_edit_mode(service: SteveCADService) -> str:
...
def provider_tool_schemas(
    ...
    *,
    ...
    if provider_engine_from_service(service) == "native":
        from SteveCADNativeProviderContext import native_provider_tool_schemas
...
def _minimal_runtime_state(service: SteveCADService) -> dict[str, Any]:
...
_COMBINED_SOURCE_INDEX_MARKER = (
    "_stevecad_deferred_combined_vibescript_program_index"
)
    ...
    if resolution.engine == "vibescript" and resolution.available and resolution.domain:
        ...
        if resolution.domain in {"partdesign", "assembly", "robot"}:
            if prepared_component_catalog is not None:
                context["_stevecad_component_catalog"] = dict(prepared_component_catalog)
            else:
                ...
                try:
                    context["_stevecad_component_catalog"] = (
                    ...
                    if str(exc) != "Component search requires an active document.":
                        raise
    context["_stevecad_debug"] = service.provider_debug_config()
    runtime_state = _minimal_runtime_state(service)
    if native_provider_surface is not None:
        from SteveCADNativeProviderContext import (
            schemas_for_native_provider_surface,
        )
        from SteveCADNativeCapabilityRegistry import _provider_schema_operations
        from SteveCADNativeTurn import native_operation_scope_digest
...
def _complete_context_for_provider(context: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if (
        ...
        and (
            editable_sources.get("_stevecad_deferred_vibescript_program_index")
            is True
    ...
    component_catalog = completed.get("_stevecad_component_catalog")
    if isinstance(component_catalog, Mapping):
        ...
        completed["_stevecad_component_catalog"] = prepared
...
def _persist_session_conversation_turn(
    ...
    *,
    ...
    if not all(callable(item) for item in (prepare, persist, accept)):
        raise RuntimeError(
            "The SteveCAD service does not implement the asynchronous "
...
def _load_conversation_for_session(
    ...
    if isinstance(accepted, dict) and accepted.get("accepted") is False:
        raise RuntimeError(
            "The active conversation changed while SteveCAD loaded its history. "
...
def _resolve_component_catalog_inputs(
    ...
    if catalog.get("schema") == "stevecad-component-catalog-snapshot-v1":
        from SteveCADComponentCatalog import component_catalog_reference_map
...
def _provider_state_payload(context: dict[str, Any]) -> dict[str, Any]:
    ...
    # STEVECAD_CONTEXT_JSON. Aero is a sibling of document/selection, not a
...
def _provider_prompt(
    ...
    *,
    ...
    if encoded_bytes > MAX_TURN_CONTEXT_JSON_BYTES:
        raise RuntimeError(
            "Deterministic SteveCAD turn-start context exceeded "
            f"{MAX_TURN_CONTEXT_JSON_BYTES} bytes ({encoded_bytes} bytes)."
        )
    from SteveCADConversationContext import RECORDS_KEY, with_handoff
    ...
    if conversation_bytes > MAX_RECENT_CONVERSATION_JSON_BYTES:
        raise RuntimeError(
            "SteveCAD recent conversation window exceeded "
    ...
    *,
    ...
    def run_worker_with_progress(
        ...
        poller = threading.Thread(
            target=poll,
            name="SteveCAD-VibeScript-worker-progress",
            daemon=True,
        )
        poller.start()
        try:
...
def _read_source_payload(
    ...
    *,
    ...
    result = {
        ...
        "_stevecad_complete_source_result": not ranged,
    }
...
def _filtered_api_payload(
    ...
    *,
    ...
    if not names and not groups:
        result["_stevecad_complete_api_result"] = True
        return result
    ...
    focused.update(
        {
            ...
            "_stevecad_complete_api_result": False,
        }
    )
...
class _SourceBoundService:
    ...
    def __init__(self, service: SteveCADService, target: _VibeScriptSourceTarget):
    ...
    def provider_document_revision(
        ...
        *,
        ...
        raise RuntimeError(
            "The SteveCAD service cannot calculate a revision for the referenced "
            "source document."
        )
...
class _DomainBoundService:
    ...
    def __init__(self, service: SteveCADService, pack: Any):
        self._service = service
        self._pack = pack
...
def _run_universal_vibescript_tool(
    ...
    *,
    ...
    def finish_source_write(result: dict[str, Any]) -> dict[str, Any]:
        result["_stevecad_source_lifecycle_result"] = True
    ...
    if tool_name == "vibescript.read_api":
        from SteveCADVibeScriptDomainRuntime import describe_api
    ...
    if tool_name == "vibescript.read_geometry":
        from SteveCADGeometryInspection import (
        ...
        try:
            ...
            payload["_stevecad_geometry_read_request"] = dict(args)
            return payload
    ...
    if tool_name == "vibescript.read_source":
        ...
        try:
            ...
            payload["_stevecad_source_read_result"] = True
            return payload
        except DomainRuntimeFailure as exc:
            return exc.payload
        except Exception as exc:
...
def apply_domain_vibescript_editor_candidate(
    ...
    *,
    ...
    from SteveCADVibeScriptDomainRuntime import accept_candidate, retain_candidate
...
def make_provider_tool_runner(
    ...
    *,
    ...
    def run(
        ...
        def finalize(payload: dict[str, Any]) -> dict[str, Any]:
            ...
            source_lifecycle_result = bool(
                payload.get("_stevecad_source_lifecycle_result")
            )
            operation_started = (
            ...
            if not bool(payload.get("ok")):
                ...
                if source_lifecycle_result:
                    ...
                    payload["_stevecad_source_lifecycle_result"] = True
            else:
            ...
            trace_payload.pop("_stevecad_image_attachment", None)
            trace_payload.pop("_stevecad_complete_source_result", None)
            trace_payload.pop("_stevecad_complete_api_result", None)
            trace_payload.pop("_stevecad_source_lifecycle_result", None)
            trace_payload.pop("_stevecad_source_read_result", None)
            trace_payload.pop("_stevecad_geometry_read_request", None)
                ...
                try:
                    ...
                    return finalize(
                        tool_failure(
                            ...
                            "UNKNOWN_TOOL",
                            "surface",
                            f"Unknown SteveCAD tool: {tool_name}",
                            requested=args,
                        )
                    )
                try:
                ...
                return finalize(
                    tool_failure(
                        ...
                        (
                            ...
                            "start a SteveCAD model or provider."
                        ),
                        requested=args,
                    )
                )
            from SteveCADDesignReview import run_design_review
        ...
        if tool_name == "vibescript.delete_object":
            from SteveCADObjectDeletion import (
                ObjectDeletionError,
                delete_exact_object,
            )
    ...
    def provider_update() -> dict[str, Any]:
        ...
        refreshed_catalog = refreshed.get("_stevecad_component_catalog")
...
def _run_session_turn(
    ...
    *,
    ...
    from SteveCADMCP import require_internal_agent
    ...
    if turn_conversation_id:
        ...
        from SteveCADConversationContext import RECORDS_KEY, SCHEMAS_KEY, TOOL_SCHEMA
    ...
    tool_runner = make_provider_tool_runner(
        ...
        turn_component_catalog=(
            ...
            if isinstance(context.get("_stevecad_component_catalog"), Mapping)
            else None
        ),
    ...
    if turn_conversation_id:
        from SteveCADConversationContext import ConversationToolRunner
...
def run_prompt(
    prompt: str,
    service: SteveCADService | None = None,
    prefer_online: bool = True,
...
def rebuild_intent_memory(
    ...
    if not active_service.intent_memory_enabled():
        raise RuntimeError("Enable Intent Memory in SteveCAD preferences first.")
    ...
    update = compile_intent_memory_update(
        ...
        debug_context={"_stevecad_debug": active_service.provider_debug_config()},
        cancellation_check=cancellation_check,
        progress_callback=progress_callback,
    )
