...
"""First-class Grok / X / xAI OAuth for SteveCAD.
...
Grok CLI in ``xai-org/grok-build``). SteveCAD does not ship a private xAI app
...
SteveCAD Grok credential directory and are never written to ordinary user.cfg.
"""
...
GROK_HOME_ENV = "STEVECAD_GROK_HOME"
...
def grok_home() -> Path:
    ...
    if sys.platform == "win32":
        ...
        return root / "SteveCAD" / "Grok"
    if sys.platform == "darwin":
        return Path.home() / "Library" / "Application Support" / "SteveCAD" / "Grok"
...
class _CallbackHandler(BaseHTTPRequestHandler):
    def do_GET(self) -> None:  # noqa: N802
        ...
        body = (
            ...
            if error_code
            else b"<html><body><p>SteveCAD Grok sign-in complete. You can close this tab.</p></body></html>"
        )
...
def start_loopback_server() -> tuple[HTTPServer, str]:
    ...
    thread = threading.Thread(
        ...
        name="SteveCAD-Grok-OAuth-Callback",
        daemon=True,
    )
    thread.start()
