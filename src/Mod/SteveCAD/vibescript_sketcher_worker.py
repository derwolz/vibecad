...
def sketch_expression_map(sketch: Any) -> dict[str, str]:
    ...
    through ``ExpressionEngine`` on every FreeCAD release supported by SteveCAD.
    ...
    if metadata is None:
        raise SketcherCandidateError(
            ...
            details={
                ...
                "correction": (
                    "Persist this exact object reference in an x-stevecad-reference input."
                ),
            },
        )
...
def _configure_sketch_support(
    ...
    if support is not None:
            ...
            raise SketcherCandidateError(
                ...
                details={
                    ...
                    "correction": (
                        ...
                        "x-stevecad-reference input schema."
                    ),
                },
            )
        from vibescript_part_worker import detached_reference_shape
        ...
        for property_name, property_value in (
            ("SteveCADWorkerReferenceDocumentUid", key[0]),
            ("SteveCADWorkerReferenceObjectName", key[1]),
            (
                "SteveCADWorkerReferenceSelection",
