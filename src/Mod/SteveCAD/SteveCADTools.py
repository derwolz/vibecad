...
"""Authoritative SteveCAD tool contracts and runtime registry."""
...
def tool_failure(
    ...
    **details: Any,
    ...
    if stage not in FAILURE_STAGES:
        raise ValueError(f"Unknown SteveCAD tool failure stage: {stage!r}")
...
class ToolSpec:
    ...
    def from_mapping(cls, raw: Mapping[str, Any]) -> "ToolSpec":
        ...
        if not name or "." not in name:
            raise ValueError(f"Invalid SteveCAD tool name: {name!r}")
...
class ToolRegistry:
    def __init__(self) -> None:
        self._tools: dict[str, SteveCADTool] = {}

    def register(self, tool: SteveCADTool) -> None:
        if tool.name in self._tools:
            raise ValueError(f"SteveCAD tool already registered: {tool.name}")
    ...
    def register_spec(
        ...
        tool = SteveCADTool(spec=ToolSpec.from_mapping(raw_spec), handler=handler)
        self.register(tool)
        return tool
