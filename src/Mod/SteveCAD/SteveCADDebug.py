...
"""Exact provider-request capture for SteveCAD context debugging."""
...
CAPTURE_SCHEMA = "stevecad-provider-request-v1"
NATIVE_DIAGNOSTIC_SCHEMA = "stevecad-native-diagnostic-v1"
CAPTURE_DIRECTORY_ENV = "STEVECAD_CONTEXT_DEBUG_DIR"
LATEST_CAPTURE_NAME = "latest.json"
LATEST_NATIVE_DIAGNOSTIC_NAME = "latest-native.json"
MAX_NATIVE_DIAGNOSTIC_JSON_BYTES = 64 * 1024


def stevecad_home() -> Path:
    configured = str(os.environ.get("STEVECAD_HOME") or "").strip()
    if configured:
        return Path(configured).expanduser()
    return Path.home() / ".stevecad"
...
def default_capture_directory() -> Path:
    ...
    if configured:
        return Path(configured).expanduser()
    return stevecad_home() / "debug" / "provider-requests"
