...
"""Agent-facing CLI for the SteveCAD local control channel.

Works two ways:

1. As a plain Python HTTP client against a running SteveCAD GUI
   (``http://127.0.0.1:8766``). No FreeCAD bindings required.
2. In-process through ``FreeCADCmd.exe`` / ``SteveCADCmd.exe`` when the GUI is
...
def _argv_for_parser(argv: list[str] | None) -> list[str]:
    ...
    extra = str(os.environ.get("STEVECAD_AGENT_ARGS") or "").strip()
    ...
    for index, item in enumerate(raw):
        ...
        if name in {"SteveCADAgentCli.py", "stevecad-agent.cmd"}:
            return raw[index + 1 :]
    return raw
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="SteveCADAgentCli",
        description=(
            "Control a running SteveCAD GUI over loopback HTTP, or run the same "
    ...
    parser.add_argument(
        "--local",
        action="store_true",
        help="Do not try the live GUI; run in this FreeCAD/SteveCAD process.",
    )
    parser.add_argument(
        "--gui-only",
        action="store_true",
    ...
    sub.add_parser("preferences", help="Show SteveCAD Preferences (GUI only).")
    ...
    ui_click_parser.add_argument(
        "--expected-process-id",
        type=int,
        help="Optional exact SteveCAD GUI process identity precondition.",
    )
    ui_click_parser.add_argument(
        "--expected-index",
        type=int,
    ...
    screenshot_parser = sub.add_parser(
        "screenshot",
        help="Capture the visible SteveCAD main window as a PNG.",
    )
...
def _endpoint_context() -> tuple[str, str, str, bool]:
    ...
    try:
        ...
        raise EndpointSafetyError(
            "The SteveCAD agent endpoint does not contain a valid host and port."
        ) from exc
    if fail_closed:
        try:
            ...
            raise EndpointSafetyError(
                "The fail-closed SteveCAD endpoint is not a valid loopback HTTP URL."
            ) from exc
        ...
        if (
            ...
            raise EndpointSafetyError(
                "The fail-closed SteveCAD endpoint must be the exact "
...
def _gui_unavailable() -> dict[str, Any]:
    return {
        ...
        "failure_stage": "transport",
        "error": (
            "No SteveCAD GUI is listening on the local agent-control port. "
            "Start SteveCAD.exe, or rerun this command through FreeCADCmd.exe "
            "without --gui-only."
        ),
    }
...
def execute(args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "error": (
                f"In-process control failed ({exc}). Start SteveCAD.exe and retry, "
