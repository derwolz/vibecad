...
"""Read the active SteveCAD ribbon surface without changing it.
...
def read_active_ribbon_surface(controller: Any | None = None) -> RibbonSurface:
    ...
    if controller is None:
        try:
            ...
            raise RibbonSurfaceError("The SteveCAD GUI is unavailable.") from exc
    ...
    if controller is None:
        raise RibbonSurfaceError("The SteveCAD ribbon controller is unavailable.")

    manifest = controller.property("SteveCADActiveSurfaceManifest")
    revision = controller.property("SteveCADActiveSurfaceRevision")
    environment = controller.property("SteveCADActiveSurfaceEnvironment")
    if environment is None:
        raise RibbonSurfaceError(
            "The SteveCAD ribbon controller did not publish its build and "
            "preference environment."
        )
    declared_surface = _required_text(
        controller.property("SteveCADActiveSurfaceId"),
        "controller.SteveCADActiveSurfaceId",
    )
