...
def bundled_python_command(package: str) -> str:
    ...
    if name in {"freecad.exe", "freecadcmd.exe", "stevecad.exe", "stevecadcmd.exe"}:
...
def require_backend(package: str, available: bool | None = None) -> None:
    ...
    raise AeroDependencyError(
        f"{package} is not installed. Install it into SteveCAD's bundled Python:\n"
        f"  {command}\n"
        "See Mod/SteveCADAero/requirements-aero.txt. Do not vendor wheels into git."
    )
