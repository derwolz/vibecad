...
def test_jsbsim_path_is_stored_on_document_and_report():
    doc = _Doc()
    obj = results.write_report(doc, _payload(), jsbsim_path="/tmp/stevecad_aero.xml")
    assert obj.JSBSimPlantPath == "/tmp/stevecad_aero.xml"
    assert getattr(doc, "JSBSimPlantPath") == "/tmp/stevecad_aero.xml"
