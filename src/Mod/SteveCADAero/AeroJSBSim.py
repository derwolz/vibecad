...
def default_output_dir(doc: Any | None = None) -> Path:
    ...
    try:
        ...
        root = Path(FreeCAD.getUserAppDataDir()) / "SteveCADAero" / "jsbsim"
        return root
    except Exception:
        return Path.home() / ".local" / "share" / "SteveCADAero" / "jsbsim"
...
def try_load_model(root: Path, model: str) -> tuple[bool, str]:
    ...
    if jsbsim is None:
        return (
            False,
            "jsbsim is not installed. Install it into SteveCAD's bundled Python:\n"
...
<fdm_config name="SteveCADAero" version="2.0" release="ALPHA"
    xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
    xsi:noNamespaceSchemaLocation="http://jsbsim.sourceforge.net/JSBSim.xsd">
  <fileheader>
    <author>SteveCAD Aero</author>
    <description>6DOF plant generated from SteveCAD Aero coefficients (not CFD).</description>
  </fileheader>
  <metrics>
    <!-- SteveCAD payload (SI): S={area:.6f} m^2, b={span:.6f} m, c={chord:.6f} m -->
  ...
  <flight_control name="SteveCAD stub"/>
  <aerodynamics>
    <axis name="LIFT">
