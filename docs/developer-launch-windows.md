# Launch the current SteveCAD checkout on Windows

Use the repo-root development launcher when you want to test the code in this
checkout without starting the SteveCAD copy installed through the normal Windows
installer.
...
```text
RUN-STEVECAD-DEV.cmd
```

`RUN-STEVECAD-DEV.cmd` delegates to the canonical launcher:

```text
Launch-SteveCAD-Dev.cmd
```
...
tree, then installs
or rebuilds `stevecad` inside this checkout's
...
The first launch is a full native SteveCAD/FreeCAD build and can take a long time.
...
checkout directory `.stevecad-dev\python-user`. This is deliberate even though
...
Finding `SteveCAD.exe` or `freecad.exe` is not sufficient. Before launch, the
...
```text
<repo>\.stevecad-dev\attestations\
```
...
The build receipt uses `stevecad.dev-build-attestation.v1`; the launch receipt
uses `stevecad.dev-launch-attestation.v1`. Together they bind:
...
- source and installed paths and SHA-256 values for `InitGui.py`,
  `SteveCADAgentControl.py`, `SteveCADAgentCli.py`, and `SteveCADGui.py` (the installed copies must be
  byte-for-byte equal to the checkout sources);
- source paths and SHA-256 values for `Launch-SteveCAD-Dev.ps1` and
  `Invoke-SteveCAD-VisibleTour.ps1`; and
...
The launcher passes only the resulting receipt paths and hashes to SteveCAD. The
...
actual visible SteveCAD process and requires that path/hash to equal the receipt
...
```powershell
.\Launch-SteveCAD-Dev.ps1 -ReleaseAttestation
```
...
remains incremental and continues to reinstall only the SteveCAD package when a
...
post-build or final probe fails, SteveCAD is not started and no external Qt files
are substituted.
...
`STEVECAD_AGENT_HOME` is rejected before a directory is created. On Windows the
...
installed SteveCAD session through the normal per-user endpoint. The launcher
prints the visible GUI PID and endpoint path, then waits for an authenticated
`/v1/status` response whose `channel` is `stevecad-agent-control` and whose
...
[stevecad-agent-control.md](stevecad-agent-control.md) for the route schemas and
security contract.
...
SteveCAD process already has access, and the explicitly authorized `/v1/run`
...
The launcher sets the literal opt-in `STEVECAD_DEV_MODE=1`. That makes this
...
1. Start `RUN-STEVECAD-DEV.cmd` (or the PowerShell launcher) from the exact
   checkout under test.
2. Confirm the visible `SteveCAD DEV • <commit>` title/status marker and the
...
the live `QMenuBar` and `SteveCADRibbonTabs`. Activation is sent to the exact Qt
...
attachment, or foreground-window activation. If the exact validated SteveCAD
...
```powershell
.\Invoke-SteveCAD-VisibleTour.ps1 -Targets @(
    'menu:File',
    'menu:Tools',
    'menu:Macro',
    'ribbon:Aero',
...
`.stevecad-dev\tours`. The receipt binds every target to the exact process ID,
...
Receipts use the versioned `stevecad.visible-operator-receipt.v1` schema. The
...
```powershell
.\Launch-SteveCAD-Dev.ps1 -SkipRebuild
```

`-SkipRebuild` does not allow the script to fall back to an installed SteveCAD or
...
$DependencyRoot = (Resolve-Path 'package/rattler-build/.pixi/bld/stevecad/<build-id>/host').Path
...
if (!(Test-Path "$NativeRoot/Mod/SteveCAD/codex_runtime")) {
    Copy-Item -LiteralPath "$PortableRoot/Mod/SteveCAD/codex_runtime" `
        -Destination "$NativeRoot/Mod/SteveCAD/codex_runtime" -Recurse
}
...
$env:STEVECAD_DEV_SOURCE_SHA = git rev-parse HEAD
$env:STEVECAD_DEV_SOURCE_TREE = git write-tree
$env:STEVECAD_AGENT_HOME = "$RepoRoot/.stevecad-dev/agent"
...
- Git
- Pixi
- the normal SteveCAD Windows build dependencies required by the Pixi package
...
root-level `SteveCAD.exe` or the Windows installer produced by the release
bundling pipeline.
