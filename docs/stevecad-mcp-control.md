# SteveCAD MCP control

SteveCAD can be controlled either by its built-in agent or by one external MCP
...
[stevecad-agent-control.md](stevecad-agent-control.md) instead of MCP.

To give the built-in agent *additional* tools from MCP servers such as
cua-driver, register them as tool servers instead; see
[stevecad-mcp-tool-servers.md](stevecad-mcp-tool-servers.md). Tool servers do not
enable this control mode.

## Connect a client

1. Open **Edit → Preferences → SteveCAD**.
...
secure storage used for SteveCAD API keys—and remains unchanged across MCP and
SteveCAD restarts. **Regenerate bearer token** is the only normal action that
...
and SteveCAD ribbon already selected by the human. In VibeScript mode these are
...
`stevecad.read_workbench` reports the active human-selected ribbon/workbench.
...
The MCP client supplies the model and reasoning. SteveCAD does not start an AI
