...
"""Generate Windows and Qt branding assets from the canonical SteveCAD mark."""
...
MASTER_MARK = ROOT / "src" / "Mod" / "SteveCAD" / "preferences-stevecad.svg"
...
def _write_icon(renderer: QSvgRenderer) -> None:
    png_path = GUI_ICONS / "stevecad-icon-256.png"
    _save_png(_render_icon(renderer, 256), png_path)
    with Image.open(png_path) as source:
        rgba = source.convert("RGBA")
        ico_path = MAIN_DIR / "stevecad.ico"
        rgba.save(
            ico_path,
            format="ICO",
            sizes=[
    ...
    installer_icon = INSTALLER_DIR / "icons" / "SteveCAD.ico"
...
def _write_header(renderer: QSvgRenderer) -> None:
    ...
    painter.setPen(TEXT)
    painter.drawText(QRectF(59, 0, 88, 57), Qt.AlignmentFlag.AlignVCenter, "SteveCAD")
    painter.end()
    _save_bmp(image, INSTALLER_DIR / "graphics" / "stevecad-header.bmp")
...
def _write_banner(renderer: QSvgRenderer) -> None:
    ...
    painter.end()
    _save_bmp(image, INSTALLER_DIR / "graphics" / "stevecad-banner.bmp")
...
def _write_splash(renderer: QSvgRenderer) -> None:
    _save_png(_splash(renderer, 568, 368), GUI_ICONS / "stevecadsplash.png")
    _save_png(_splash(renderer, 1136, 736), GUI_ICONS / "stevecadsplash_2x.png")
...
def _about(renderer: QSvgRenderer, *, development: bool) -> QImage:
    ...
    painter.drawText(QRectF(190, 26, 335, 62), Qt.AlignmentFlag.AlignVCenter, "SteveCAD")
    painter.setPen(LIGHT_BLUE)
...
def _write_about(renderer: QSvgRenderer) -> None:
    _save_png(_about(renderer, development=False), GUI_ICONS / "stevecadabout.png")
    _save_png(_about(renderer, development=True), GUI_ICONS / "stevecadaboutdev.png")
...
def main() -> None:
    ...
    if not renderer.isValid():
        raise RuntimeError(f"Invalid SteveCAD master mark: {MASTER_MARK}")
    shutil.copyfile(MASTER_MARK, GUI_ICONS / "stevecad.svg")
    _write_icon(renderer)
    _write_header(renderer)
    _write_banner(renderer)
    _write_splash(renderer)
    _write_about(renderer)
    print("Generated SteveCAD branding assets.")
