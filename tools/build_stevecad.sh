...
build_dir="${STEVECAD_BUILD_DIR:-${repo_root}/build/release}"
build_type="${STEVECAD_BUILD_TYPE:-Release}"
generator="${STEVECAD_CMAKE_GENERATOR:-Ninja}"
jobs="${STEVECAD_BUILD_JOBS:-$(nproc)}"
stevecad_requirements="${repo_root}/src/Mod/SteveCAD/requirements.txt"
...
usage() {
    cat <<EOF
Usage: tools/build_stevecad.sh [options]
...
Environment:
  STEVECAD_BUILD_DIR       Build directory. Default: <repo>/build/release
  STEVECAD_BUILD_TYPE      CMake build type. Default: Release
  STEVECAD_CMAKE_GENERATOR CMake generator. Default: Ninja
  STEVECAD_BUILD_JOBS      Parallel build jobs. Default: nproc
EOF
}
...
if [[ -f "${stevecad_requirements}" ]]; then
    mkdir -p "${build_dir}/Ext"
    python3 -m pip install \
        --disable-pip-version-check \
        --upgrade \
        --target "${build_dir}/Ext" \
        --requirement "${stevecad_requirements}"
...
"${build_dir}/bin/FreeCADCmd" -c "import anthropic, keyring, jsonschema; print('SteveCAD Python dependencies import OK')"
...
echo "SteveCAD build complete: ${build_dir}"
