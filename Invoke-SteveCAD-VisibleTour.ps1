...
<#
.SYNOPSIS
Runs a visible, semantically verified SteveCAD UI tour with a virtual cursor.

.DESCRIPTION
The tour draws its own plain cyan cursor over SteveCAD. It never moves or
...
function New-SteveCADTourReceiptPath {
    [CmdletBinding()]
    param(
    ...
    $TourDirectory = Join-Path $FullRepositoryRoot ".stevecad-dev\tours"
...
function New-SteveCADTourReceiptPayload {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory = $true)]
    ...
    return [pscustomobject][ordered]@{
        schema = "stevecad.visible-operator-receipt.v1"
        ok = $true
        channel = "stevecad-visible-operator"
        custom_cursor = "SteveCADVirtualCursor"
        virtual_cursor_color = "cyan"
        physical_cursor_control = "none"
...
function Write-SteveCADTourReceipt {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory = $true)]
        [ValidateNotNull()]
    ...
    try {
        ...
        if (Test-Path -LiteralPath $ResolvedReceiptPath) {
            throw "Refusing to overwrite existing SteveCAD tour receipt '$ResolvedReceiptPath'."
        }
        throw
    }
...
if ($env:OS -ne "Windows_NT") {
    throw "Invoke-SteveCAD-VisibleTour.ps1 requires Windows for its virtual-cursor overlay."
}
$RepositoryRoot = [IO.Path]::GetFullPath($PSScriptRoot)
$AgentHome = [IO.Path]::GetFullPath(
    (Join-Path $RepositoryRoot ".stevecad-dev\agent")
)
...
$ExecutableCandidates = @(
    (Join-Path $EnvironmentBin "SteveCAD.exe"),
...
if (-not $ExpectedExecutable) {
    throw "This checkout has no built SteveCAD GUI executable. Run .\Launch-SteveCAD-Dev.ps1 first."
}
...
function Get-CheckoutAgentConnection {
    if (-not (Test-Path -LiteralPath $EndpointPath -PathType Leaf)) {
        throw "Missing checkout-scoped agent endpoint: $EndpointPath. Run .\Launch-SteveCAD-Dev.ps1 first."
    }
    ...
    if ($BaseUri.Scheme -ne "http" -or $BaseUri.Host -notin @("127.0.0.1", "localhost", "::1")) {
        throw "Refusing non-loopback SteveCAD endpoint $BaseUri."
    }
...
function Invoke-SteveCADAgentGet {
...
function Invoke-SteveCADAgentPost {
    param(
...
$Status = Invoke-SteveCADAgentGet -Route "/v1/status"
if (
    -not $Status.ok -or
    $Status.channel -ne "stevecad-agent-control" -or
    -not $Status.gui_up
) {
    throw "The checkout-scoped SteveCAD GUI is not ready for virtual operation."
}

function Get-ExpectedSteveCADProcess {
    if ($SteveCADProcessId -gt 0) {
        $Candidates = @(Get-Process -Id $SteveCADProcessId -ErrorAction Stop)
    }
    else {
        $Candidates = @(
            Get-Process -Name "SteveCAD", "freecad" -ErrorAction SilentlyContinue |
                Where-Object {
                    try {
    ...
    if ($Candidates.Count -ne 1) {
        throw "Expected exactly one visible SteveCAD process from this checkout; found $($Candidates.Count)."
    }
    $Process = $Candidates[0]
    $Process.Refresh()
    if (-not $Process.Responding) {
        throw "SteveCAD PID $($Process.Id) is not responding."
    }
    ...
    if (-not $ActualExecutable.Equals($ExpectedExecutable, [StringComparison]::OrdinalIgnoreCase)) {
        throw "PID $($Process.Id) is not this checkout's SteveCAD executable."
    }
    return $Process
}

$SteveCADProcess = Get-ExpectedSteveCADProcess

if (-not ("SteveCADVisibleOperator.SteveCADVirtualCursor" -as [type])) {
...
namespace SteveCADVisibleOperator
{
    ...
    public sealed class SteveCADVirtualCursor : Form
    {
        private bool pressed;

        public SteveCADVirtualCursor()
        {
...
function Get-SteveCADWindowBounds {
    param([Parameter(Mandatory = $true)][long] $WindowHandle)
    if ($WindowHandle -le 0) {
        throw "SteveCAD did not report a valid QMainWindow handle."
    }
    [SteveCADVisibleOperator.RECT]$Bounds = New-Object SteveCADVisibleOperator.RECT
    $Window = [IntPtr]$WindowHandle
    [uint32]$OwnerProcessId = 0
    [SteveCADVisibleOperator.NativeWindow]::GetWindowThreadProcessId(
        $Window,
        [ref]$OwnerProcessId
    ) | Out-Null
    if ([int]$OwnerProcessId -ne $SteveCADProcess.Id) {
        throw "Qt window handle $WindowHandle does not belong to exact SteveCAD PID $($SteveCADProcess.Id)."
    }
    if (-not [SteveCADVisibleOperator.NativeWindow]::GetWindowRect($Window, [ref]$Bounds)) {
        throw "Windows could not report the exact SteveCAD window bounds."
    }
    return $Bounds
}

function Restore-SteveCADWindowIfMinimized {
    param([Parameter(Mandatory = $true)][long] $WindowHandle)
    ...
    $null = Get-SteveCADWindowBounds -WindowHandle $WindowHandle
    $Window = [IntPtr]$WindowHandle
    if (-not [SteveCADVisibleOperator.NativeWindow]::IsIconic($Window)) {
        return $false
    }
    [SteveCADVisibleOperator.NativeWindow]::ShowWindow(
        $Window,
        [SteveCADVisibleOperator.NativeWindow]::SW_SHOWNOACTIVATE
    ) | Out-Null
    ...
    throw "The exact SteveCAD window remained minimized after SW_SHOWNOACTIVATE."
}
...
function Assert-TargetInsideExactWindow {
    ...
    $Bounds = Get-SteveCADWindowBounds -WindowHandle $WindowHandle
    if (
        $X -lt $Bounds.Left -or
        ...
        throw "Semantic target ($X,$Y) is outside the exact SteveCAD window."
    }
}

function Get-SteveCADUiSnapshot {
    param([Parameter(Mandatory = $true)][string] $Kind)
    $Route = if ($Kind -eq "menu") { "/v1/ui/menus" } else { "/v1/ui/ribbon" }
    $Snapshot = Invoke-SteveCADAgentGet -Route $Route
    if (-not $Snapshot.ok) {
        throw "SteveCAD $Kind snapshot failed: $($Snapshot.failure_code): $($Snapshot.error)"
    }
    if (-not $Snapshot.visible -or [int]$Snapshot.process_id -ne $SteveCADProcess.Id) {
        throw "The live $Kind target is not visible or belongs to another process."
    }
    return $Snapshot
}

function Get-SteveCADDefaultTourTargets {
    $MenuSnapshot = Get-SteveCADUiSnapshot -Kind "menu"
    $RibbonSnapshot = Get-SteveCADUiSnapshot -Kind "ribbon"
...
if (-not $Targets -or @($Targets).Count -eq 0) {
    $Targets = @(Get-SteveCADDefaultTourTargets)
}
if (@($Targets).Count -eq 0) {
    throw "The live SteveCAD window exposed no enabled menu or ribbon targets."
}

function Resolve-SteveCADUiTarget {
    ...
    $Snapshot = Get-SteveCADUiSnapshot -Kind $Kind
...
function Move-SteveCADVirtualCursor {
    param(
        [Parameter(Mandatory = $true)][int] $X,
        [Parameter(Mandatory = $true)][int] $Y,
        [Parameter(Mandatory = $true)] $Cursor,
...
$InitialSnapshot = Get-SteveCADUiSnapshot -Kind "ribbon"
if (
    Restore-SteveCADWindowIfMinimized `
        -WindowHandle ([long]$InitialSnapshot.window_handle)
) {
    $InitialSnapshot = Get-SteveCADUiSnapshot -Kind "ribbon"
}
$Bounds = Get-SteveCADWindowBounds -WindowHandle ([long]$InitialSnapshot.window_handle)
$CursorState = [pscustomobject]@{
    X = [int]($Bounds.Left + 24)
    Y = [int]($Bounds.Top + 84)
}
$Receipts = [Collections.Generic.List[object]]::new()
$VirtualCursor = New-Object SteveCADVisibleOperator.SteveCADVirtualCursor
...
try {
    ...
    foreach ($RequestedTarget in @($Targets)) {
        $Target = Resolve-SteveCADUiTarget -Specification ([string]$RequestedTarget)
        Assert-TargetInsideExactWindow `
            -X $Target.X `
            -Y $Target.Y `
            -WindowHandle $Target.WindowHandle
        Move-SteveCADVirtualCursor `
            -X $Target.X `
            -Y $Target.Y `
        ...
        $Click = Invoke-SteveCADAgentPost -Route "/v1/ui/click" -Body @{
            kind = $Target.Kind
            text = $Target.Text
            expected_index = $Target.Index
            expected_process_id = $SteveCADProcess.Id
        }
        ...
        if ($Target.Kind -eq "menu") {
            ...
            $Verified = Get-SteveCADUiSnapshot -Kind "menu"
            $VerifiedMenu = @(
                $Verified.menus |
        ...
        $Receipts.Add([pscustomobject]@{
            ...
            exact_process_id = $SteveCADProcess.Id
...
if (-not $ReceiptPath) {
    $ReceiptPath = New-SteveCADTourReceiptPath -RepositoryRoot $RepositoryRoot
}
elseif (-not [IO.Path]::IsPathRooted($ReceiptPath)) {
    $ReceiptPath = Join-Path $RepositoryRoot $ReceiptPath
}
$ResolvedReceiptPath = [IO.Path]::GetFullPath($ReceiptPath)
$Payload = New-SteveCADTourReceiptPayload `
    -ProcessId $SteveCADProcess.Id `
    -Executable $ExpectedExecutable `
    -Targets @($Receipts) `
    -ReceiptPath $ResolvedReceiptPath
$ReceiptJson = Write-SteveCADTourReceipt `
    -Payload $Payload `
    -ReceiptPath $ResolvedReceiptPath
$ReceiptJson
